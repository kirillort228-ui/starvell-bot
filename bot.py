...
def extract_order_buyer_username(order: dict) -> str:
    buyer = order.get("buyer") or order.get("customer") or order.get("client") or {}
...
def is_sale_order(order: dict, my_username: str | None = None) -> bool:
    """
    Tries to separate sales from purchases.
    If Starvell response has seller/buyer fields, sales usually contain buyer data for seller account.
    """
...
def is_successful_sale(order: dict) -> bool:
    ...
    if not raw:
        return True
    return raw in ("COMPLETED", "DONE", "FINISHED", "CLOSED", "PAID", "SUCCESS", "SUCCESSFUL")
...
def format_sales_stats(username: str, orders_data: dict, *, days: int = 30) -> str:
    ...
    for order in all_orders:
        if not is_last_30_days(extract_order_date(order)):
            continue
        if not is_sale_order(order, username):
            continue
...
def format_month_stats(username: str, chats_data: dict, orders_data: dict) -> str:
    ...
    recent_sales = [
        ...
        if is_sale_order(order, username) and is_successful_sale(order)
    ]
