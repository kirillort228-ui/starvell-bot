...
async def build_confirm_reminder_text(user_id: int) -> str:
    ...
    return (
        ...
        f"Настроено товаров: <b>{len(offer_ids)}</b>\n\n"
        "<b>Выбранные товары:</b>\n"
        f"{preview_text}\n\n"
        "Нажми <b>«Выбрать товар»</b>, затем у выбранного товара можно будет настроить:\n"
        "• время отправки\n"
        "• дни отправки\n"
        "• текст напоминания\n\n"
...
async def find_unconfirmed_chats_for_account(account: dict) -> list[dict]:
    """Ищет чаты/заказы, где оплата была, но подтверждения получения ещё нет."""
    client = StarvellClient(cookie=account.cookie, proxy_url=account.proxy_url)

    chats = []
    try:
        chats_data = await client.get_chats()
        chats = extract_chats(chats_data) if isinstance(chats_data, dict) else (chats_data or [])
    except Exception:
        chats = []
    ...
    for chat in chats if isinstance(chats, list) else []:
        ...
        is_paid = any(marker in status_text for marker in paid_markers)
        is_confirmed = any(marker in status_text for marker in confirmed_markers)

        # При нехватке полей читаем полную историю чата.
        messages = []
        if not is_paid or not is_confirmed:
            try:
            ...
            is_paid = is_paid or any(marker in combined for marker in paid_markers)
            is_confirmed = is_confirmed or any(
                marker in combined for marker in confirmed_markers
            )
            ...
            result.append(
                {
                    ...
                    "title": str(
                        chat.get("title")
                        or chat.get("buyerUsername")
                        or chat.get("username")
                        or chat.get("orderId")
                        or f"Чат {chat_id[:8]}"
                    ),
                    "orderId": chat.get("orderId"),
                    "raw": chat,
                }
            )
...
@dp.callback_query(F.data == "reminder:choose_product")
async def confirm_reminder_choose_product(callback: CallbackQuery, state: FSMContext) -> None:
    # Сразу подтверждаем нажатие, иначе Telegram прекращает показывать реакцию,
    # пока бот последовательно проверяет десятки категорий.
    await callback.answer("Ищу твои товары…")
    try:
        await callback.message.edit_text(
            "🔎 <b>Ищу твои товары</b>\n\n"
            "Проверяю все сохранённые категории Starvell и личные данные аккаунта. "
    ...
    try:
        _account, offers = await load_all_seller_products_for_message_settings(callback.from_user.id)
    except Exception as exc:
        await callback.message.edit_text(
    ...
    if not offers:
        await callback.message.edit_text(
            "❌ Твои активные товары не найдены ни в категориях, ни в личных данных профиля Starvell.",
            reply_markup=confirm_reminder_menu_keyboard(),
        )
        return

    await state.update_data(confirm_reminder_offer_picker=offers)
    await callback.message.edit_text(
        "📦 <b>Выбери товар для напоминания о подтверждении</b>\n\n"
        f"Найдено товаров: <b>{len(offers)}</b>",
        reply_markup=build_message_products_keyboard(offers, action_prefix="reminderpick"),
    )


@dp.callback_query(F.data.startswith("reminderpick:"))
async def confirm_reminder_pick_product(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    command = parts[1]
    data = await state.get_data()
    offers = data.get("confirm_reminder_offer_picker") or []

    if command == "page" and len(parts) >= 3:
        page = int(parts[2])
        await callback.message.edit_reply_markup(
            reply_markup=build_message_products_keyboard(offers, action_prefix="reminderpick", page=page)
        )
        await callback.answer()
        return
    ...
    title = get_offer_title(selected)
    await add_confirm_reminder_offer(callback.from_user.id, offer_id)
    await db.set_account_setting(callback.from_user.id, f"confirm_reminder_title:{offer_id}", title)
    await clean_callback_answer(
        callback,
        await build_confirm_reminder_product_text(callback.from_user.id, offer_id, title),
        reply_markup=confirm_reminder_product_menu_keyboard(offer_id),
        alert_text="Товар выбран",
    )


@dp.callback_query(F.data.startswith("reminder:product:"))
async def confirm_reminder_product_menu(callback: CallbackQuery) -> None:
    offer_id = callback.data.split(":", 2)[2]
    title = await db.get_account_setting(callback.from_user.id, f"confirm_reminder_title:{offer_id}", "")
    await clean_callback_answer(
        callback,
        await build_confirm_reminder_product_text(callback.from_user.id, offer_id, title),
        reply_markup=confirm_reminder_product_menu_keyboard(offer_id),
...
async def confirm_reminder_time_menu_legacy(callback: CallbackQuery) -> None:
    await callback.answer("Сначала выбери товар.", show_alert=True)


@dp.callback_query(F.data.startswith("reminder:time_menu:"))
async def confirm_reminder_time_menu(callback: CallbackQuery) -> None:
    ...
    await clean_callback_answer(
        ...
        reply_markup=confirm_reminder_time_keyboard(offer_id),
    )


@dp.callback_query(F.data == "reminder:time_custom")
async def confirm_reminder_time_custom_legacy(callback: CallbackQuery) -> None:
    await callback.answer("Сначала выбери товар.", show_alert=True)


@dp.callback_query(F.data.startswith("reminder:time_custom:"))
async def confirm_reminder_time_custom(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.update_data(confirm_reminder_offer_id=offer_id)
    await callback.message.answer(
...
async def confirm_reminder_time_set(callback: CallbackQuery) -> None:
    ...
    await db.set_account_setting(callback.from_user.id, f"confirm_reminder_time:{offer_id}", time_value)
    await clean_callback_answer(
        callback,
        await build_confirm_reminder_product_text(callback.from_user.id, offer_id),
        reply_markup=confirm_reminder_product_menu_keyboard(offer_id),
...
async def confirm_reminder_time_custom_save(message: Message, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    offer_id = str(data.get("confirm_reminder_offer_id") or "")
    await db.set_account_setting(message.from_user.id, f"confirm_reminder_time:{offer_id}", value)
    await state.clear()
    await message.answer(
        await build_confirm_reminder_product_text(message.from_user.id, offer_id),
        reply_markup=confirm_reminder_product_menu_keyboard(offer_id),
    )


@dp.callback_query(F.data == "reminder:period_menu")
async def confirm_reminder_period_menu_legacy(callback: CallbackQuery) -> None:
    await callback.answer("Сначала выбери товар.", show_alert=True)


@dp.callback_query(F.data.startswith("reminder:period_menu:"))
async def confirm_reminder_period_menu(callback: CallbackQuery) -> None:
    ...
    await clean_callback_answer(
        ...
        reply_markup=confirm_reminder_period_keyboard(offer_id),
    )
...
async def confirm_reminder_period_set(callback: CallbackQuery) -> None:
    ...
    if days not in {"1", "2", "3", "7"}:
        await callback.answer("Неверный период", show_alert=True)
        return
    await db.set_account_setting(callback.from_user.id, f"confirm_reminder_period_days:{offer_id}", days)
    await clean_callback_answer(
        callback,
        await build_confirm_reminder_product_text(callback.from_user.id, offer_id),
        reply_markup=confirm_reminder_product_menu_keyboard(offer_id),
...
@dp.callback_query(F.data.startswith("reminder:edit:"))
async def confirm_reminder_product_text_edit(callback: CallbackQuery, state: FSMContext) -> None:
    offer_id = callback.data.split(":", 2)[2]
    await state.set_state(ConfirmReminderProductText.waiting_text)
    await state.update_data(confirm_reminder_offer_id=offer_id)
    await callback.message.answer(
        "✍️ Отправь новый текст напоминания одним сообщением.\n\n"
...
@dp.callback_query(F.data.startswith("reminder:clear:"))
async def confirm_reminder_product_text_clear(callback: CallbackQuery) -> None:
    offer_id = callback.data.split(":", 2)[2]
    await db.set_account_setting(callback.from_user.id, f"confirm_reminder_text:{offer_id}", "")
    await clean_callback_answer(
        callback,
        await build_confirm_reminder_product_text(callback.from_user.id, offer_id),
        reply_markup=confirm_reminder_product_menu_keyboard(offer_id),
...
@dp.message(ConfirmReminderProductText.waiting_text)
async def confirm_reminder_product_text_save(message: Message, state: FSMContext) -> None:
    value = (message.text or "").strip()
    if not value:
    ...
    data = await state.get_data()
    offer_id = str(data.get("confirm_reminder_offer_id") or "")
    await db.set_account_setting(message.from_user.id, f"confirm_reminder_text:{offer_id}", value)
    await state.clear()
    await message.answer(
        await build_confirm_reminder_product_text(message.from_user.id, offer_id),
        reply_markup=confirm_reminder_product_menu_keyboard(offer_id),
...
async def save_confirm_reminder_text(
    ...
    await message.answer(
        ...
        "Бот будет использовать введённый текст для этого чата."
    )
