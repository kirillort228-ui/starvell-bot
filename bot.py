...
def format_top_sellers_list(sellers: list[dict]) -> str:
    ...
    text = "🏆 <b>Топ продавцов STARVELL</b>\n\n"
    for index, seller in enumerate(sellers[:20], start=1):
        username = seller_username(seller)
        rating = seller_rating(seller)
        reviews = seller_reviews(seller)
        rating_text = f"{float(rating):.2f}" if isinstance(rating, (int, float)) else (str(rating) if rating else "—")
        text += f"{index}. <b>{escape(username)}</b> — ⭐ {rating_text} | отзывов: {reviews}\n"

    if len(sellers) > 20:
        text += f"\nПоказано 20 из {len(sellers)} продавцов."
    return text
...
def extract_order_amount_kopecks(order: dict) -> int:
    for key in ("amount", "price", "rubAmount", "total", "totalRubAmount", "cost"):
        value = order.get(key)
...
async def seller_top1000(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer("🔄 Получаю топ продавцов STARVELL...")
    account = accounts[0]
    client = StarvellClient(cookie=account.cookie, proxy_url=account.proxy_url)
    try:
        data = await client.get_top_sellers()
        ...
        await callback.message.answer(
            "❌ <b>Не удалось автоматически получить топ продавцов.</b>\n\n"
            f"<code>{error}</code>\n\n"
