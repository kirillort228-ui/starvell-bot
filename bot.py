...
async def scan_user_products(user_id: int) -> list[dict]:
    ...
    found: list[dict] = []
    try:
        # 1. Account offers if Starvell exposes this endpoint.
        try:
            offers_data = await asyncio.wait_for(client.get_account_offers(), timeout=20)
            found.extend(extract_products_from_offers_data(offers_data))
        except Exception:
            pass

        # 2. Profile JSON often contains pageProps.offers, like trade/profile data.
        if account.username:
            try:
                profile_data = await asyncio.wait_for(client.get_profile(account.username), timeout=20)
                found.extend(extract_products_from_offers_data(profile_data))
            except Exception:
                pass

        # 3. Chats/orders can contain offerDetails.
        try:
            chats_data = await asyncio.wait_for(client.get_chats(), timeout=20)
            found.extend(extract_products_from_chats_data(chats_data))
        except Exception:
            pass

        # 4. Public trade pages fallback. It keeps only current user's offers when userId is present.
        try:
            groups = await asyncio.wait_for(client.collect_bump_groups_from_open_trade(), timeout=25)
            for game_id, category_ids in groups.items():
                for category_id in category_ids:
                    found.append({
                        "offerPublicId": "",
                        "categoryId": str(category_id),
                        "gameId": str(game_id),
                        "title": f"Категория {category_id} / gameId {game_id}",
                    })
        except Exception:
            pass
    finally:
...
async def auto_delivery_scan(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not items:
        await callback.message.answer(
            "❌ Не удалось найти товары автоматически.\n\n"
            "Можно ввести offerPublicId или categoryIds вручную.",
