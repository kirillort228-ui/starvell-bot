...
TOP_FIX_VERSION = "top-reviews-profile-json-v2"
...
def format_top_sellers_list(sellers: list[dict]) -> str:
    ...
    text = (
        ...
        "Топ собран с открытых категорий Starvell, а отзывы проверяются через профиль продавца.\n\n"
    )
...
async def scan_user_products(user_id: int) -> list[dict]:
    ...
    dedup: dict[str, dict] = {}
    for item in found:
        key = item.get("offerPublicId") or f"cat:{item.get('categoryId')}"
...
async def build_product_select_text(user_id: int) -> str:
    ...
    raise_categories = await db.get_account_setting(user_id, "auto_raise_category_ids", "335")
    return (
        ...
        "<b>Автоподнятие работает только по категориям:</b>\n"
        f"<code>{escape(str(raise_categories or 'не заданы'))}</code>\n\n"
        "Важно: Starvell endpoint для поднятия, который мы нашли, принимает <code>gameId</code> и <code>categoryIds</code>, "
        "а не отдельный offerPublicId. Поэтому для автоподнятия оставлены только categoryIds."
    )
...
async def get_auto_raise_config(user_id: int) -> dict:
    ...
    game_id_raw = await db.get_account_setting(user_id, "auto_raise_game_id", "31")
    categories_raw = await db.get_account_setting(user_id, "auto_raise_category_ids", "335")
...
async def build_auto_raise_text(user_id: int, account_id: int | None = None) -> str:
    ...
    return (
        ...
        f"gameId: <code>{cfg['game_id'] or 'не задан'}</code>\n"
        f"categoryIds: <code>{','.join(map(str, cfg['category_ids'])) if cfg['category_ids'] else 'не заданы'}</code>\n"
        ...
        f"<i>Версия: {AUTO_RAISE_VERSION}</i>\n"
        "Используется реальный endpoint Starvell: <code>POST /api/offers/bump</code>."
    )


async def run_auto_raise_for_account(account) -> str:
    cfg = await get_auto_raise_config(account.user_id)
    if not cfg["game_id"] or not cfg["category_ids"]:
        raise StarvellApiError("Сначала укажи gameId и categoryIds.")

    client = StarvellClient(cookie=account.cookie, proxy_url=account.proxy_url)
    try:
        result = await client.bump_offers(cfg["game_id"], cfg["category_ids"])
...
def build_support_text() -> str:
    username, url = get_support_contacts()
    contact = f"@{username}" if username else "не указан"
    text = (
        "🆘 <b>Поддержка</b>\n\n"
        "Здесь можно обратиться по вопросам:\n"
        "• подключение Starvell-аккаунта;\n"
        "• cookie/session;\n"
        "• прокси;\n"
        "• подписка и баланс бота;\n"
        "• ошибки уведомлений и статистики.\n\n"
        f"👤 Контакт поддержки: <b>{escape(contact)}</b>\n\n"
        "Перед обращением лучше отправить скрин ошибки и кратко написать, что нажимал."
    )
    if not username and not url:
        text += (
            "\n\n⚠️ Поддержка ещё не настроена. "
            "Добавь в <code>.env</code> строку <code>SUPPORT_USERNAME=твой_username</code>."
        )
    return text
...
async def auto_raise_set_categories(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        "Для твоего запроса из Network сейчас подходит: <code>335</code>\n"
        "Если категорий несколько: <code>335,336</code>",
        reply_markup=cancel_keyboard(),
    )
    await callback.answer()
...
async def auto_raise_save_categories(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        ids = []
    if not ids:
        await message.answer("❌ Нужно отправить одно или несколько чисел. Пример: <code>335</code>")
        return
...
async def auto_raise_run_now(callback: CallbackQuery) -> None:
    ...
    if not account:
        ...
        await callback.answer()
        return
    await callback.message.answer("🚀 Поднимаю лоты через Starvell API...")
    try:
        ...
        await callback.message.answer(
            ...
            "Проверь gameId/categoryIds или пришли Response запроса bump.",
            reply_markup=auto_raise_settings_keyboard(await db.get_bool_account_setting(callback.from_user.id, "auto_raise_lots")),
        )
    await callback.answer()
...
async def product_select_scan(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not items:
        await callback.message.answer(
            ...
            "• <b>categoryIds</b> для автоответчика и автоподнятия.\n\n"
            "categoryId из твоего bump-запроса сейчас: <code>335</code>.",
            reply_markup=product_select_menu_keyboard(),
        )
        return
    ...
    text += "Нажми на товар ниже, чтобы добавить его в фильтр автоответчика. Если у товара есть categoryId, его также можно использовать для автоподнятия."
...
async def product_select_set_categories(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        "Пример для твоего bump-запроса: <code>335</code>\n\n"
...
async def autoraise_categories_debug(message: Message) -> None:
    await message.answer(
        ...
        f"Текущие categoryIds: <code>{escape(str(await db.get_account_setting(message.from_user.id, 'auto_raise_category_ids', '335') or 'не заданы'))}</code>"
    )
...
async def product_select_debug(message: Message) -> None:
    await message.answer(
        ...
        f"Автоподнятие categoryIds: <code>{escape(str(await db.get_account_setting(message.from_user.id, 'auto_raise_category_ids', '335') or 'не заданы'))}</code>"
    )
...
async def top_debug(message: Message) -> None:
    await message.answer(
        ...
        "Отзывы теперь берутся из public profile JSON продавца, а не только из карточек категорий."
    )
