...
async def build_auto_raise_text(user_id: int, account_id: int | None = None) -> str:
    ...
    return (
        ...
        f"<i>Версия: {AUTO_RAISE_PROFILE_MODE_VERSION}</i>\n"
        "Бот определяет владельца по подключённому профилю, находит его активные лоты "
        "и отправляет в поднятие категории, в которых они размещены."
    )
...
async def auto_delivery_add_product(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        f"offerPublicId: <code>{escape(offer_id or '—')}</code>\n\n"
