...
from chat_watcher import baseline_account_messages, check_one_account, extract_chats, extract_user, get_chat_link, get_interlocutor, format_notification_message, CHAT_FEATURES_VERSION
...
def format_compact_profit_stats(
    ...
    *,
    ...
    Only withdrawal commission is shown, fixed at 2.9%.
    The listing-bump and KOSell rows are intentionally omitted.
    Final profit uses the seller amount after Starvell commission.
    """
    ...
    gross_revenue = sum(extract_order_gross_amount_kopecks(order) for order in sales)

    # basePrice / sellerAmount is treated as the seller amount after Starvell commission.
    after_starvell_commission = sum(extract_order_amount_kopecks(order) for order in sales)

    # Withdrawal fee is calculated from the amount available to the seller.
    withdrawal_fee = round(after_starvell_commission * 0.029)

    profit_after_commissions = max(0, after_starvell_commission - withdrawal_fee)
    ...
    return (
        ...
        f"💰 Выручка (грязными): <b>{format_rub(gross_revenue)}</b>\n\n"
        f"➖ Комиссия за вывод (2,9%): <b>{format_rub(withdrawal_fee)}</b>\n\n"
...
async def build_auto_delivery_text(user_id: int) -> str:
    ...
    offer_ids = await db.get_account_setting(user_id, "auto_delivery_offer_ids", "")
    category_ids = await db.get_account_setting(user_id, "auto_delivery_category_ids", "")
    return (
        ...
        f"offerPublicId: <code>{escape(offer_ids) if offer_ids else 'не выбраны'}</code>\n"
        f"categoryIds: <code>{escape(category_ids) if category_ids else 'не выбраны'}</code>\n\n"
...
async def build_auto_raise_text(user_id: int, account_id: int | None = None) -> str:
    ...
    return (
        ...
        f"Режим: <b>автоматический поиск категорий лотов</b>\n"
        f"Запасные значения: gameId <code>{cfg['game_id'] or 16}</code>, categoryIds <code>{','.join(map(str, cfg['category_ids'])) if cfg['category_ids'] else '208'}</code>\n"
        ...
        f"<i>Версия: {AUTO_BUMP_ALL_CATEGORIES_VERSION}</i>\n"
        "При нажатии «Поднять лоты сейчас» бот сам ищет активные лоты и отправляет в bump все categoryIds найденной игры."
    )
...
async def run_auto_raise_for_account(account) -> str:
    ...
    try:
        ...
        # Fallback: use saved/default values only if automatic scan found nothing.
        if not groups and cfg["game_id"] and cfg["category_ids"]:
            groups = {int(cfg["game_id"]): set(int(x) for x in cfg["category_ids"])}

        if not groups:
            raise StarvellApiError(
                "Не удалось автоматически найти категории активных лотов. "
                "Открой страницу своих лотов на Starvell и пришли trade.json/profile JSON без Cookie/session."
            )

        results = []
        for game_id, category_ids_set in sorted(groups.items()):
            category_ids = sorted(int(x) for x in category_ids_set if int(x) > 0)
            if not category_ids:
                continue

            try:
                result = await client.bump_offers(int(game_id), category_ids)
                results.append(f"gameId={game_id}, categoryIds={category_ids}: {str(result)[:500]}")
            except Exception as grouped_error:
                # If Starvell says cooldown, do not spam separate requests.
                if extract_bump_cooldown_seconds(grouped_error) is not None:
                    raise
                # If a big category list fails for another reason, try categories one by one.
                split_results = []
                for category_id in category_ids:
                    try:
                        one_result = await client.bump_offers(int(game_id), [int(category_id)])
                        split_results.append(f"{category_id}: OK {str(one_result)[:180]}")
                    except Exception as one_error:
                        if extract_bump_cooldown_seconds(one_error) is not None:
                            raise
                        split_results.append(f"{category_id}: ERROR {type(one_error).__name__}: {str(one_error)[:180]}")
                    await asyncio.sleep(0.7)
                results.append(f"gameId={game_id}, split categories: " + "; ".join(split_results))
            await asyncio.sleep(0.7)

        if not results:
            raise StarvellApiError("Категории найдены, но список для поднятия пуст.")
...
async def auto_raise_run_now(callback: CallbackQuery) -> None:
    ...
    if not account:
        ...
        await callback.answer()
        return
    await callback.message.answer("🚀 Ищу категории активных лотов и поднимаю их через Starvell API...")
    try:
        ...
        if cooldown_seconds is not None:
            ...
            await callback.message.answer(
                ...
                "Пришли Response trade.json/profile JSON без Cookie/session, если автоопределение не нашло лоты.",
                reply_markup=auto_raise_settings_keyboard(await db.get_bool_account_setting(callback.from_user.id, "auto_raise_lots")),
            )
    await callback.answer()
...
async def auto_delivery_scan(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer("Ищу товары...")
    items = await scan_user_products(callback.from_user.id)
    await state.update_data(auto_delivery_scan_items=items)
    if not items:
        await callback.message.answer(
            "❌ Не удалось найти активные товары продавца.\n\n"
            "Бот проверил аккаунт, профиль и категории всех обнаруженных игр, но Starvell не вернул список лотов.",
    ...
    text = "📋 <b>Выбери товар для автовыдачи</b>\n\n"
    for idx, item in enumerate(items[:12], start=1):
        text += (
            f"{idx}. <b>{escape(str(item.get('title') or 'Товар'))}</b>\n"
            f"   offerPublicId: <code>{escape(str(item.get('offerPublicId') or '—'))}</code>\n"
    ...
    await callback.message.answer(text, reply_markup=auto_delivery_products_keyboard(items))
...
async def auto_delivery_add_product(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if offer_id:
        current = csv_set_from_text(
            await db.get_account_setting(callback.from_user.id, "auto_delivery_offer_ids", "")
        )
        current.add(offer_id)
    ...
    if category_id:
        current_categories = csv_set_from_text(
            await db.get_account_setting(callback.from_user.id, "auto_delivery_category_ids", "")
        )
...
async def auto_delivery_clear_products(callback: CallbackQuery) -> None:
    ...
    await clean_callback_answer(
        callback,
        "🧹 Выбор товаров для автовыдачи очищен.\n\n"
        + await build_auto_delivery_text(callback.from_user.id),
        reply_markup=auto_delivery_menu_keyboard(
            await db.get_bool_account_setting(callback.from_user.id, "auto_delivery")
        ),
