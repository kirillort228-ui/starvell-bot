...
async def load_all_seller_products_for_message_settings(
    ...
    # Сначала используем прокси аккаунта. Если Starvell не отвечает через него,
    # автоматически повторяем поиск напрямую без прокси.
    offers: list[dict] = []
    ...
    if not offers and account.proxy_url:
        ...
        try:
            ...
            if first_error is not None:
                raise RuntimeError(
                    "Starvell не ответил ни через прокси, ни напрямую при поиске товаров. "
