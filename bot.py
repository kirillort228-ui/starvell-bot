...
async def build_auto_raise_text(user_id: int, account_id: int | None = None) -> str:
    ...
    selected = await get_auto_raise_selected_products(user_id)
    last_at = None
    last_result = None
    if account_id:
    ...
    if selected:
        ...
        for index, item in enumerate(selected[:10], start=1):
            title = escape(str(item.get("title") or "Товар"))
            game = escape(str(item.get("gameName") or item.get("gameId") or "—"))
            category = escape(str(item.get("categoryName") or item.get("categoryId") or "—"))
            selected_lines.append(f"{index}. {title} — {game} / {category}")
        selected_text = "\\n".join(selected_lines)
        if len(selected) > 10:
            selected_text += f"\\n…и ещё {len(selected) - 10}"
    else:
        selected_text = "товары не выбраны"
    ...
    return (
        ...
        f"<b>Товары для автоподнятия:</b>\\n{selected_text}\\n\\n"
        f"Последний запуск: <code>{escape(str(last_at or 'ещё не запускалось'))}</code>\\n"
        f"Результат: <code>{escape(str(last_result or '—'))}</code>\\n\\n"
        f"<i>Версия: {AUTO_RAISE_PRODUCT_SELECTION_VERSION}</i>\\n"
        "Автоподнятие работает по принципу автовыдачи: сначала выбираются товары, "
        "затем бот поднимает только категории выбранных товаров."
    )


async def run_auto_raise_for_account(account) -> str:
    selected = await get_auto_raise_selected_products(account.user_id)
    if not selected:
        raise StarvellApiError(
            "Сначала выбери товары для автоподнятия через кнопку «📋 Выбрать товары»."
        )

    groups = group_selected_products_for_bump(selected)
    if not groups:
        raise StarvellApiError(
            "У выбранных товаров не найдены gameId/categoryId. Очисти выбор и выбери товары заново."
        )
    ...
    try:
        ...
        for game_id, category_ids_set in sorted(groups.items()):
            ...
            try:
                ...
                results.append(
                    f"✅ gameId={game_id}, categoryIds={category_ids}: {str(result)[:350]}"
                )
                ...
                for category_id in category_ids:
                    try:
                        ...
                        split_results.append(
                            f"{category_id}: ❌ {type(one_error).__name__}: {str(one_error)[:120]}"
                        )
        ...
        if not results:
            raise StarvellApiError("Не удалось сформировать запрос для выбранных товаров.")
...
async def auto_raise_run_now(callback: CallbackQuery) -> None:
    ...
    if not account:
        ...
        await callback.answer()
        return
    await callback.message.answer("🚀 Поднимаю выбранные товары через Starvell API...")
    try:
        ...
        if cooldown_seconds is not None:
            ...
            await callback.message.answer(
                ...
                "Проверь, что товары выбраны в разделе автоподнятия.",
                reply_markup=auto_raise_settings_keyboard(await db.get_bool_account_setting(callback.from_user.id, "auto_raise_lots")),
            )
    await callback.answer()
