...
TOP_FIX_VERSION = "top-reviews-profile-only-v3"
...
def format_top_sellers_list(sellers: list[dict]) -> str:
    ...
    text = (
        ...
        "Топ собран с открытых категорий Starvell, а отзывы берутся только из подтверждённого профиля продавца.\n\n"
    )
    for index, seller in enumerate(sellers[:30], start=1):
        ...
        source_note = " ⚠️ профиль не подтвердился" if seller.get("source") == "profile-unverified" else ""
...
async def build_product_select_text(user_id: int) -> str:
    ...
    raise_categories = await db.get_account_setting(user_id, "auto_raise_category_ids", "208")
    return (
...
async def build_auto_raise_text(user_id: int, account_id: int | None = None) -> str:
    ...
    return (
        ...
        f"<i>Версия: {AUTO_BUMP_DETECT_VERSION}</i>\n"
        "При нажатии «Поднять лоты сейчас» бот сам ищет активные лоты, берёт их gameId/categoryId и вызывает <code>POST /api/offers/bump</code>."
    )
...
async def product_select_scan(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not items:
        await callback.message.answer(
            ...
            "categoryId из твоего bump-запроса сейчас: <code>208</code>.",
            reply_markup=product_select_menu_keyboard(),
        )
        return
...
async def auto_bump_detect_debug(message: Message) -> None:
    ...
    await message.answer(
        "🧪 <b>Диагностика автоподнятия</b>\n\n"
        f"Версия: <code>{AUTO_BUMP_DETECT_VERSION}</code>\n"
        "Режим: автоматический поиск категорий при запуске поднятия.\n\n"
        f"Найдено:\n{groups_text}"
    )
...
async def top_debug(message: Message) -> None:
    await message.answer(
        ...
        "Отзывы теперь берутся только из подтверждённого профиля продавца. Если профиль не подтвердился, число отзывов из карточки категории не используется."
    )
