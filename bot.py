...
from aiogram.types import CallbackQuery, Message, InlineKeyboardButton, InlineKeyboardMarkup, TelegramObject
from aiogram.client.default import DefaultBotProperties
from aiogram.enums import ParseMode
from aiogram.exceptions import TelegramBadRequest

from chat_watcher import baseline_account_messages, check_one_account, check_auto_confirm_open_orders, extract_chats, extract_user, get_chat_link, get_interlocutor, format_notification_message, get_notification_type, extract_chat_product_info, is_purchase_paid_event, CHAT_FEATURES_VERSION
...
from keyboards import (
    ...
    confirm_reminder_product_menu_keyboard,
    confirm_reminder_time_keyboard,
    main_keyboard,
    no_subscription_keyboard,
    required_channel_keyboard,
...
def get_required_channel_url() -> str:
    if config.required_channel_url:
        return config.required_channel_url
    channel_id = str(config.required_channel_id or "").strip()
    if channel_id.startswith("@"):
        return f"https://t.me/{channel_id[1:]}"
    return ""


async def is_required_channel_member(user_id: int) -> bool:
    channel_id = str(config.required_channel_id or "").strip()
    if not channel_id:
        return True
...
async def send_required_channel_prompt(message: Message) -> None:
    await message.answer(
        "📢 <b>Для использования бота нужна подписка на канал</b>\n\n"
        "Сначала подпишись на канал, затем нажми "
        "<b>«Проверить подписку»</b>.\n\n"
        "До проверки подписки нельзя пользоваться функциями бота "
        "и покупать подписку.",
        reply_markup=required_channel_keyboard(get_required_channel_url()),
    )
...
class RequiredChannelMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: TelegramObject, data: dict):
        ...
        if isinstance(event, CallbackQuery):
            if str(event.data or "") == "channel:check":
                return await handler(event, data)
            user_id = event.from_user.id
            if await is_required_channel_member(user_id):
                return await handler(event, data)
            await event.answer("Сначала подпишись на канал.", show_alert=True)
            if event.message:
                await send_required_channel_prompt(event.message)
            return None
        ...
        if isinstance(event, Message):
            ...
            if await is_required_channel_member(user_id):
                return await handler(event, data)
            await send_required_channel_prompt(event)
            return None
...
class ConfirmReminderProductText(StatesGroup):
    waiting_text = State()
...
# Все категории, переданные пользователем для автоподнятия Starvell.
# Формат: gameId -> categoryIds.
AUTO_RAISE_CATEGORY_CATALOG: dict[int, list[int]] = {
    1: [38],     # Roblox — Аккаунты
    10: [136],   # Brawl Stars — Brawl Pass
    11: [144],   # Clash Royale — Pass Royale
    12: [153],   # Clash of Clans — Gold Pass
    13: [201],   # PUBG Mobile — UC
    14: [182],   # Telegram — Звёзды
    15: [189],   # Discord — Nitro
    16: [207],   # Steam — Пополнение баланса
    17: [226],   # Minecraft — Аккаунты
    18: [238],   # Hytale — Ключи
    19: [245],   # Standoff 2 — Голда
    20: [255],   # Fortnite — В-баксы
    21: [261],   # Mobile Legends — Алмазы
    22: [270],   # Black Russia — Вирты
    23: [278],   # Valorant — VPoints
    24: [284],   # Genshin Impact — Кристаллы сотворения
    25: [298],   # ChatGPT — Подписка
    26: [302],   # Call of Duty Mobile — CP
    27: [308],   # ARC Raiders — Ключи
    28: [318],   # Free Fire — Алмазы
    29: [325],   # Spotify — Premium
    30: [330],   # TikTok — Монеты
    31: [335],   # YouTube — Premium
    32: [340],   # App Store — Подарочные карты
    33: [346],   # Xbox — Пополнение баланса
    34: [351],   # PlayStation — Пополнение бумажника
    35: [353],   # Counter-Strike 2 — Prime
    36: [361],   # Rust — Ключи
    37: [370],   # Overwatch — Монеты
    38: [376],   # Claude — Подписка
    39: [381],   # Gemini — Подписка
    40: [394],   # Dota 2 — Dota Plus
    41: [401],   # Battle.net — Пополнение баланса
    48: [440],   # GTA 5 RP — Вирты
    49: [446],   # Apex Legends — Монеты
}

AUTO_RAISE_CATEGORY_NAMES: dict[tuple[int, int], str] = {
    (1, 38): "Roblox — Аккаунты",
    (10, 136): "Brawl Stars — Brawl Pass",
    (11, 144): "Clash Royale — Pass Royale",
    (12, 153): "Clash of Clans — Gold Pass",
...
AUTO_RAISE_ALL_CATEGORIES_VERSION = "autoraise-all-user-categories-v1"


# Полный каталог страниц trade.json, переданных пользователем.
# Используется автовыдачей для перебора всех категорий и поиска товаров продавца.
AUTO_DELIVERY_CATEGORY_PAGES: list[tuple[int, int, str, str, str]] = [
    (1, 38, "roblox", "accounts", "Roblox — Аккаунты"),
    (10, 136, "brawl-stars", "brawl-pass", "Brawl Stars — Brawl Pass"),
    (11, 144, "clash-royale", "pass-royale", "Clash Royale — Pass Royale"),
    (12, 153, "clash-of-clans", "gold-pass", "Clash of Clans — Gold Pass"),
    (13, 201, "pubg-mobile", "uc", "PUBG Mobile — UC"),
    (14, 182, "telegram", "stars", "Telegram — Звёзды"),
    (15, 189, "discord", "nitro", "Discord — Nitro"),
    (16, 207, "steam", "balance", "Steam — Пополнение баланса"),
    (17, 226, "minecraft", "accounts", "Minecraft — Аккаунты"),
    (18, 238, "hytale", "keys", "Hytale — Ключи"),
    (19, 245, "standoff-2", "gold", "Standoff 2 — Голда"),
    (20, 255, "fortnite", "vbucks", "Fortnite — В-баксы"),
    (21, 261, "mobile-legends", "diamonds", "Mobile Legends — Алмазы"),
    (22, 270, "black-russia", "virts", "Black Russia — Вирты"),
    (23, 278, "valorant", "vp", "Valorant — VPoints"),
    (24, 284, "genshin-impact", "crystals", "Genshin Impact — Кристаллы сотворения"),
    (25, 298, "chatgpt", "subscription", "ChatGPT — Подписка"),
    (26, 302, "call-of-duty-mobile", "cp", "Call of Duty Mobile — CP"),
    (27, 308, "arc-raiders", "keys", "ARC Raiders — Ключи"),
    (28, 318, "free-fire", "diamonds", "Free Fire — Алмазы"),
    (29, 325, "spotify", "premium", "Spotify — Premium"),
    (30, 330, "tiktok", "coins", "TikTok — Монеты"),
    (31, 335, "youtube", "premium", "YouTube — Premium"),
    (32, 340, "app-store", "gift-cards", "App Store — Подарочные карты"),
    (33, 346, "xbox", "balance", "Xbox — Пополнение баланса"),
    (34, 351, "playstation", "balance", "PlayStation — Пополнение бумажника"),
    (35, 353, "counter-strike-2", "prime", "Counter-Strike 2 — Prime"),
    (36, 361, "rust", "keys", "Rust — Ключи"),
    (37, 370, "overwatch", "coins", "Overwatch — Монеты"),
    (38, 376, "claude", "subscription", "Claude — Подписка"),
    (39, 381, "gemini", "subscription", "Gemini — Подписка"),
    (40, 394, "dota-2", "dotaplus", "Dota 2 — Dota Plus"),
    (41, 401, "battle-net", "balance", "Battle.net — Пополнение баланса"),
    (48, 440, "gta-5-rp", "virts", "GTA 5 RP — Вирты"),
    (49, 446, "apex-legends", "coins", "Apex Legends — Монеты"),
]

AUTO_DELIVERY_FULL_SCAN_VERSION = "autodelivery-full-category-scan-v1"

MESSAGE_AUTOMATIONS_PRODUCT_SELECTION_VERSION = "message-automations-product-selection-v1"
PRODUCT_DISCOVERY_FALLBACK_VERSION = "category-scan-plus-profile-fallback-v5"
CHOOSE_PRODUCT_BUTTON_FIX_VERSION = "choose-product-immediate-response-v6"
BUILD_ID_FALLBACK_VERSION = "next-build-id-fallback-v7"
DIRECT_RETRY_VERSION = "proxy-then-direct-retry-v8"
PROFILE_TIMEOUT_FIX_VERSION = "profile-timeout-does-not-stop-scan-v9"
CONFIRM_REMINDER_CHAT_SEARCH_VERSION = "confirm-reminder-search-unconfirmed-chats-v10"
OPEN_ORDER_FAST_SEARCH_VERSION = "open-order-chats-single-request-v11"
OPEN_ORDER_RUNTIME_FIX_VERSION = "open-order-real-post-timeout-v12"
OPEN_ORDER_HEADERS_FIX_VERSION = "open-order-browser-headers-v13"
ACCOUNT_LOOKUP_FIX_VERSION = "list-user-accounts-primary-v14"
INLINE_KEYBOARD_IMPORT_FIX_VERSION = "inline-keyboard-import-v15"
CONFIRM_REMINDER_SAVE_FIX_VERSION = "confirm-reminder-save-v16"
MESSAGE_TEXTS_UI_VERSION = "message-texts-ui-v17"
REMOVE_AUTOREPOST_AUTORESPONDER_VERSION = "remove-autorepost-autoresponder-v18"
AUTO_CONFIRM_PRODUCT_SELECTION_VERSION = "auto-confirm-product-selection-v19"
AUTO_MESSAGES_RELIABILITY_VERSION = "auto-messages-reliability-v19"
PROFILE_PRODUCTS_JSON_VERSION = "profile-user-profile-offers-v20"
PROFILE_BUILD_ID_RETRY_VERSION = "profile-build-id-retry-v21"
REVIEW_TRIGGER_FIX_VERSION = "review-requires-explicit-five-stars-v22"
REQUIRED_CHANNEL_VERSION = "required-channel-membership-v23"
AUTO_CONFIRM_RUNTIME_STATUS_VERSION = "auto-confirm-endpoint-not-yet-connected-v24"
AUTO_MESSAGE_DIAGNOSTICS_VERSION = "auto-message-diagnostics-v24"
AUTO_CONFIRM_RUNTIME_VERSION = "auto-confirm-mark-seller-completed-v25"
AUTO_CONFIRM_OPEN_ORDER_VERSION = "auto-confirm-direct-open-orders-v26"
AUTO_MESSAGES_CHAT_JSON_VERSION = "auto-messages-chat-json-v27"
ADMIN_DASHBOARD_VERSION = "admin-dashboard-invoice-protection-v28"
...
async def build_auto_confirm_text(user_id: int) -> str:
    enabled = await db.get_bool_account_setting(user_id, "auto_confirm")
    selected = csv_set_from_text(
        await db.get_account_setting(user_id, "auto_confirm_offer_ids", "")
    )
    return (
        "🤝 <b>Автоподтверждение</b>\n\n"
        f"Статус: {'✅ включено' if enabled else '❌ выключено'}\n"
        f"Выбрано товаров: <b>{len(selected)}</b>\n\n"
        "Автоподтверждение будет применяться только к выбранным товарам.\n"
        "Если список пустой — функция не применяется ни к одному товару."
    )
...
async def build_account_setting_text_view(user_id: int, key: str) -> str:
    title = account_setting_title(key)
    current_text = await db.get_account_setting(user_id, key, "")
    current_text_rendered = escape(current_text) if current_text and current_text.strip() else "текст не задан"

    lines = [f"{title}", ""]

    if key == "auto_responder":
        enabled = await db.get_bool_account_setting(user_id, "auto_responder_enabled", default=True)
        offer_ids = csv_set_from_text(await db.get_account_setting(user_id, "auto_responder_offer_ids", ""))
        category_ids = csv_set_from_text(await db.get_account_setting(user_id, "auto_responder_category_ids", ""))

        lines.extend(
            [
                f"Статус: {'✅ включен' if enabled else '❌ выключен'}",
                f"Товаров выбрано: <b>{len(offer_ids)}</b>",
                f"Категорий выбрано: <b>{len(category_ids)}</b>",
                "",
            ]
        )
    ...
    lines.extend(
        [
            "<b>Текущий текст:</b>",
            current_text_rendered,
            "",
            "Ниже можно изменить текст, очистить его"
            + (" или выбрать товары." if key == "auto_responder" else "."),
        ]
    )
    return "\n".join(lines)
...
def get_confirm_reminder_offer_ids_from_text(value: str | None) -> list[str]:
    return sorted(csv_set_from_text(value or ""))


async def get_confirm_reminder_offer_ids(user_id: int) -> list[str]:
    value = await db.get_account_setting(user_id, "confirm_reminder_offer_ids", "")
    return get_confirm_reminder_offer_ids_from_text(value)


async def add_confirm_reminder_offer(user_id: int, offer_id: str) -> None:
    current = set(await get_confirm_reminder_offer_ids(user_id))
    current.add(str(offer_id))
    await db.set_account_setting(user_id, "confirm_reminder_offer_ids", ",".join(sorted(current)))
...
async def build_confirm_reminder_text(user_id: int) -> str:
    ...
    offer_ids = await get_confirm_reminder_offer_ids(user_id)
    preview = []
    for offer_id in offer_ids[:5]:
        title = await db.get_account_setting(user_id, f"confirm_reminder_title:{offer_id}", "")
        preview.append(f"• {escape(title or offer_id)}")
    preview_text = "\n".join(preview) if preview else "пока ни один товар не выбран"
    if len(offer_ids) > 5:
        preview_text += f"\n• и ещё {len(offer_ids) - 5}"

    return (
        "⏰ <b>Напоминание о подтверждении</b>\n\n"
        f"Статус: {'✅ включено' if enabled else '❌ выключено'}\n"
        f"Задержка после заказа: <b>только спустя 24 часа</b>\n"
        "Нажми <b>«Найти чаты без подтверждения»</b>.\n"
        "Бот проверит твои чаты и покажет те, где заказ оплачен, но покупатель ещё не подтвердил получение.\n\n"
        "Переменные: <code>{username}</code>, <code>{order_id}</code>, <code>{seller}</code>."
    )


async def build_confirm_reminder_product_text(user_id: int, offer_id: str, product_title: str = "") -> str:
    time_value = await db.get_account_setting(user_id, f"confirm_reminder_time:{offer_id}", "13:00")
    period_days = int(await db.get_account_setting(user_id, f"confirm_reminder_period_days:{offer_id}", "1") or "1")
    text_value = await db.get_account_setting(user_id, f"confirm_reminder_text:{offer_id}", "")
    title = product_title or await db.get_account_setting(user_id, f"confirm_reminder_title:{offer_id}", "") or offer_id
    if period_days == 1:
...
def extract_order_offer_public_id(order: dict) -> str:
    if not isinstance(order, dict):
        return ""
    for value in (
        order.get("offerPublicId"),
        ((order.get("offer") or {}).get("publicId") if isinstance(order.get("offer"), dict) else None),
        ((order.get("offer") or {}).get("offerPublicId") if isinstance(order.get("offer"), dict) else None),
        find_nested_first(order.get("offer") if isinstance(order.get("offer"), (dict, list)) else {}, ("offerPublicId", "publicId", "uuid")),
        find_nested_first(order.get("product") if isinstance(order.get("product"), (dict, list)) else {}, ("offerPublicId", "publicId", "uuid")),
        find_nested_first(order, ("offerPublicId",)),
    ):
        if value not in (None, ""):
            return str(value)
    return ""
...
async def process_confirm_reminders_for_account(account) -> None:
    ...
    configured_offer_ids = set(await get_confirm_reminder_offer_ids(account.user_id))
    if not configured_offer_ids:
        return
    ...
    try:
        ...
        for order in orders:
            ...
            offer_public_id = extract_order_offer_public_id(order)
            if not offer_public_id or offer_public_id not in configured_offer_ids:
                continue

            time_value = await db.get_account_setting(account.user_id, f"confirm_reminder_time:{offer_public_id}", "13:00")
            if not should_run_confirm_reminder_now(time_value):
                continue

            period_days = int(await db.get_account_setting(account.user_id, f"confirm_reminder_period_days:{offer_public_id}", "1") or "1")
            template = await db.get_account_setting(account.user_id, f"confirm_reminder_text:{offer_public_id}", "")
...
async def build_auto_raise_text(user_id: int, account_id: int | None = None) -> str:
    ...
    return (
        ...
        f"Категории: <b>{sum(len(v) for v in AUTO_RAISE_CATEGORY_CATALOG.values())} загружено</b>\n"
...
async def run_auto_raise_for_account(account) -> str:
    ...
    try:
        ...
        # Основной список — все категории, которые пользователь передал вручную.
        # Дополнительно добавляем категории активных лотов, найденные в профиле,
        # чтобы новые товары тоже не пропускались.
        groups: dict[int, set[int]] = {
            int(game_id): {int(category_id) for category_id in category_ids}
            for game_id, category_ids in AUTO_RAISE_CATEGORY_CATALOG.items()
        }

        active_offers: list[dict] = []
        try:
            active_offers = await client.collect_seller_offers_from_category_pages(
            username,
            AUTO_DELIVERY_CATEGORY_PAGES,
        )
        except Exception:
            active_offers = []
        ...
        results = [
            f"Профиль: {username}",
            f"Категорий из каталога: {sum(len(v) for v in AUTO_RAISE_CATEGORY_CATALOG.values())}",
            f"Активных лотов найдено дополнительно: {len(active_offers)}",
            f"Всего игр для поднятия: {len(groups)}",
            f"Всего категорий для поднятия: {sum(len(v) for v in groups.values())}",
        ]
        ...
        for game_id, category_ids_set in sorted(groups.items()):
            ...
            try:
                ...
                titles = [
                    AUTO_RAISE_CATEGORY_NAMES.get((game_id, category_id), f"{game_id}/{category_id}")
                    for category_id in category_ids
                ]
                results.append("✅ " + ", ".join(titles))
...
async def watcher_loop() -> None:
    ...
    logger = logging.getLogger("starvell.watcher")
    logger.warning("WATCHER_STARTED interval=%s", config.check_interval_seconds)

    while True:
        try:
            accounts = await db.list_enabled_accounts()
            logger.warning("WATCHER_CYCLE accounts=%s", len(accounts))

            for account in accounts:
                try:
                    await check_auto_confirm_open_orders(db, account)
                except Exception as exc:
                    logger.exception(
                        "AUTO_CONFIRM_FAILED account_id=%s error=%s",
                        account.id,
                        exc,
                    )
                ...
                try:
                    ...
                    logger.exception(
                        "CHAT_WATCHER_FAILED account_id=%s error=%s",
                        account.id,
                        exc,
                    )
            ...
            logger.exception("WATCHER_LOOP_FAILED error=%s", exc)
...
class MessageAutomationStates(StatesGroup):
    waiting_for_product_message = State()
    waiting_for_confirm_reminder_text = State()


async def load_all_seller_products_for_message_settings(
    telegram_user_id: int,
) -> tuple[dict, list[dict]]:
    account = await get_primary_account(telegram_user_id)
    if not account:
        raise ValueError("Сначала подключи аккаунт Starvell.")

    username = str(account.username or "").strip()
    if not username:
        raise ValueError("У подключённого аккаунта не найден username.")

    # Основной источник — JSON страницы профиля:
    # pageProps.bff.userProfileOffers[].offers
    offers: list[dict] = []
    first_error: Exception | None = None

    client = StarvellClient(cookie=account.cookie, proxy_url=account.proxy_url)
    try:
        offers = await client.collect_profile_page_offers(username)
        if not offers:
            offers = await client.collect_seller_offers_from_category_pages(
                username,
                AUTO_DELIVERY_CATEGORY_PAGES,
            )
    except Exception as exc:
    ...
    if not offers and account.proxy_url:
        direct_client = StarvellClient(cookie=account.cookie, proxy_url=None)
        try:
            offers = await direct_client.collect_profile_page_offers(username)
            if not offers:
                offers = await direct_client.collect_seller_offers_from_category_pages(
                    username,
                    AUTO_DELIVERY_CATEGORY_PAGES,
                )
            ...
            if first_error is not None:
                raise RuntimeError(
                    "Не удалось получить товары профиля ни через прокси, ни напрямую. "
                    f"Через прокси: {first_error}. Напрямую: {direct_error}"
                ) from direct_error
            raise
        finally:
            await direct_client.close()

    if not offers and first_error is not None:
        raise first_error

    # Стабильная сортировка: категория, название, offerPublicId.
    def offer_title(item: dict) -> str:
        descriptions = item.get("descriptions") or {}
        rus = descriptions.get("rus") or {}
        return str(
            rus.get("briefDescription")
            or rus.get("description")
            or item.get("_categoryTitle")
            or "Товар"
        ).strip()

    offers.sort(
        key=lambda item: (
            str(item.get("_categoryTitle") or ""),
            offer_title(item).lower(),
            str(item.get("publicId") or item.get("offerPublicId") or ""),
        )
    )
    return account, offers


def build_message_products_keyboard(
    offers: list[dict],
    *,
    action_prefix: str,
    page: int = 0,
    ...
    for offer in offers[start : start + page_size]:
        public_id = str(offer.get("publicId") or offer.get("offerPublicId") or "")
        descriptions = offer.get("descriptions") or {}
        rus = descriptions.get("rus") or {}
        title = str(
            rus.get("briefDescription")
            or offer.get("_categoryTitle")
            or "Товар"
        ).strip()
        if len(title) > 48:
            title = title[:45] + "…"
        rows.append(
            [
                InlineKeyboardButton(
                    text=title,
                    callback_data=f"{action_prefix}:select:{public_id}",
                )
            ]
        )
    ...
    if page > 0:
        nav.append(
            InlineKeyboardButton(
                text="⬅️",
                callback_data=f"{action_prefix}:page:{page - 1}",
            )
        )
    nav.append(
        InlineKeyboardButton(
            text=f"{page + 1}/{total_pages}",
            callback_data=f"{action_prefix}:noop",
        )
    )
    ...
    rows.append(nav)
    rows.append(
        [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"{action_prefix}:back")]
    )
    return InlineKeyboardMarkup(inline_keyboard=rows)


def get_offer_title(offer: dict) -> str:
    descriptions = offer.get("descriptions") or {}
    rus = descriptions.get("rus") or {}
    return str(
        rus.get("briefDescription")
...
async def find_unconfirmed_chats_for_account(account) -> list[dict]:
    """Получает открытые чаты продавца одним быстрым API-запросом."""
    chats: list[dict] = []
    proxy_error: Exception | None = None

    client = StarvellClient(cookie=account.cookie, proxy_url=account.proxy_url)
    try:
        chats = await client.get_open_order_chats(
            offset=0,
            limit=50,
            role="SELLER",
        )
    ...
    # Если прокси завис или не пропускает этот endpoint, повторяем напрямую.
    if not chats and account.proxy_url:
        direct_client = StarvellClient(cookie=account.cookie, proxy_url=None)
        try:
            chats = await direct_client.get_open_order_chats(
                offset=0,
                limit=50,
                role="SELLER",
            )
            ...
            raise RuntimeError(
                "Не удалось получить открытые заказы ни через прокси, ни напрямую. "
                f"Через прокси: {proxy_error}. Напрямую: {direct_error}"
            ) from direct_error
        finally:
            await direct_client.close()
    elif proxy_error is not None and not chats:
        raise proxy_error

    result: list[dict] = []
    my_username = str(getattr(account, "username", "") or "").lower()

    for chat in chats:
        chat_id = str(chat.get("id") or chat.get("chatId") or "").strip()
        if not chat_id:
            continue
        ...
        for participant in participants:
            ...
            if username and username.lower() != my_username:
        ...
        title = buyer_username or f"Чат {chat_id[:8]}"
        if last_content:
            title = f"{title} — {last_content[:45]}"
        ...
        result.append(
            {
                ...
                "unreadMessageCount": int(chat.get("unreadMessageCount") or 0),
                "raw": chat,
            }
        )
...
def build_unconfirmed_chats_keyboard(chats: list[dict]) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    for item in chats[:50]:
        chat_id = str(item.get("chatId") or "")
        title = str(item.get("title") or f"Чат {chat_id[:8]}")
        if len(title) > 52:
            title = title[:49] + "…"
        rows.append(
            [
                InlineKeyboardButton(
                    text=title,
                    callback_data=f"confirm_reminder_chat:{chat_id}",
                )
            ]
        )
    rows.append(
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="confirm_reminder_back")]
    )
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def create_topup_request(user_id: int, amount_kopecks: int, message: Message) -> None:
    allowed, reason = await db.can_create_topup_request(user_id)
    if not allowed:
        await message.answer("⏳ <b>Новый счёт пока нельзя создать</b>\n\n" + escape(reason))
        return
    await db.touch_user(user_id, message.from_user.username, message.from_user.first_name)
    ...
    if config.admin_id:
        try:
            await bot.send_message(
                ...
                f"🆔 Заявка: <code>{request_id}</code>\n"
                f"🧾 Invoice ID: <code>{invoice_id}</code>\n"
                f"👤 Пользователь: <b>{escape(message.from_user.first_name or 'не указано')}</b>\n"
                f"🔗 Username: <b>{escape('@' + message.from_user.username if message.from_user.username else 'не указан')}</b>\n"
...
async def check_crypto_topup_request(request_id: int, *, notify_user: bool = True) -> bool:
    ...
    if config.admin_id:
        try:
            user_info = await db.get_user_public_info(request.user_id)
            username_value = user_info.get("telegram_username")
            username = "@" + username_value if username_value else "не указан"
            first_name = user_info.get("first_name") or "не указано"
            await bot.send_message(
                config.admin_id,
                "✅ <b>Crypto Bot оплата зачислена</b>\n\n"
                f"🆔 Заявка: <code>{request.id}</code>\n"
                f"🧾 Invoice ID: <code>{invoice_id}</code>\n"
                f"👤 Пользователь: <b>{escape(str(first_name))}</b>\n"
                f"🔗 Username: <b>{escape(username)}</b>\n"
                f"🆔 Telegram ID: <code>{request.user_id}</code>\n"
                f"📌 Статус: <b>оплачено и зачислено</b>\n"
...
@dp.callback_query(F.data == "channel:check")
async def required_channel_check(callback: CallbackQuery) -> None:
    if await is_required_channel_member(callback.from_user.id):
        await callback.answer("Подписка подтверждена", show_alert=True)
        await callback.message.answer(
            "✅ <b>Подписка на канал подтверждена!</b>\n\n"
            "Теперь можно пользоваться ботом и покупать подписку.",
            reply_markup=main_keyboard,
        )
        return

    await callback.answer(
        "Подписка пока не найдена. Подпишись и повтори проверку.",
        show_alert=True,
    )
    await send_required_channel_prompt(callback.message)


async def build_admin_dashboard_text() -> str:
    st = await db.get_admin_dashboard_stats()
    return (
        "🛡 <b>Панель владельца</b>\n\n"
        "🤖 <b>Состояние бота</b>\n"
        f"╭ Пользователей: <b>{st['total_users']}</b>\n"
        f"├ Активных подписок: <b>{st['active_subscriptions']}</b>\n"
        f"├ Starvell-аккаунтов: <b>{st['connected_accounts']}</b>\n"
        f"╰ Уведомления включены: <b>{st['notifications_enabled']}</b>\n\n"
        "💳 <b>Платежи</b>\n"
        f"╭ Всего счетов: <b>{st['total_invoices']}</b>\n"
        f"├ Оплачено: <b>{st['paid_invoices']}</b>\n"
        f"├ Ожидают: <b>{st['pending_invoices']}</b>\n"
        f"├ Отклонено: <b>{st['rejected_invoices']}</b>\n"
        f"╰ Получено: <b>{format_rub(st['paid_amount_kopecks'])}</b>"
    )
...
async def admin_dashboard_command(message: Message) -> None:
    if not config.admin_id or message.from_user.id != config.admin_id:
        return
    await message.answer(await build_admin_dashboard_text(), reply_markup=admin_dashboard_keyboard())

@dp.callback_query(F.data == "admin:dashboard")
async def admin_dashboard_refresh(callback: CallbackQuery) -> None:
    if not config.admin_id or callback.from_user.id != config.admin_id:
        await callback.answer("Нет доступа", show_alert=True); return
    await clean_callback_answer(callback, await build_admin_dashboard_text(), reply_markup=admin_dashboard_keyboard(), alert_text="Обновлено")

@dp.callback_query(F.data == "admin:users")
async def admin_recent_users(callback: CallbackQuery) -> None:
    if not config.admin_id or callback.from_user.id != config.admin_id:
        await callback.answer("Нет доступа", show_alert=True); return
    st = await db.get_admin_dashboard_stats(); rows=[]
    for u in st['recent_users']:
        username = '@'+u['telegram_username'] if u.get('telegram_username') else 'без username'
        rows.append(f"• <b>{escape(str(u.get('first_name') or 'Без имени'))}</b> — {escape(username)} (<code>{u['user_id']}</code>)")
    await clean_callback_answer(callback, "👥 <b>Последние пользователи</b>\n\n" + ('\n'.join(rows) if rows else 'Пока нет пользователей.'), reply_markup=admin_dashboard_keyboard())

@dp.callback_query(F.data == "admin:invoices")
async def admin_invoice_summary(callback: CallbackQuery) -> None:
    if not config.admin_id or callback.from_user.id != config.admin_id:
        await callback.answer("Нет доступа", show_alert=True); return
    st = await db.get_admin_dashboard_stats()
    await clean_callback_answer(callback, "💳 <b>Счета</b>\n\n" + f"Всего: <b>{st['total_invoices']}</b>\nОплачено: <b>{st['paid_invoices']}</b>\nОжидают: <b>{st['pending_invoices']}</b>\nОтклонено: <b>{st['rejected_invoices']}</b>\nПолучено: <b>{format_rub(st['paid_amount_kopecks'])}</b>", reply_markup=admin_dashboard_keyboard())
...
@dp.message(F.text == "/automation_debug")
async def automation_debug(message: Message) -> None:
    auto_confirm_enabled = await db.get_bool_account_setting(message.from_user.id, "auto_confirm")
    auto_confirm_products = csv_set_from_text(
        await db.get_account_setting(message.from_user.id, "auto_confirm_offer_ids", "")
    )
    ...
    for key in (
        ...
        value = await db.get_account_setting(message.from_user.id, key, "")
        if value and value.strip():
            configured_messages.append(key)

    await message.answer(
        "🧪 <b>Диагностика автоматизаций</b>\n\n"
        f"Автоподтверждение включено: <b>{'да' if auto_confirm_enabled else 'нет'}</b>\n"
        f"Товаров выбрано: <b>{len(auto_confirm_products)}</b>\n"
        "Реальный POST подтверждения заказа: <b>подключён</b>\n\n"
        f"Настроено автоматических текстов: <b>{len(configured_messages)}</b>\n"
        f"Ключи: <code>{escape(', '.join(configured_messages) or 'нет')}</code>\n\n"
        "После нового события открой Railway Logs и найди строки:\n"
        "<code>STARVELL_NEW_MESSAGE</code>\n"
        "<code>AUTO_MESSAGE</code>"
    )
...
async def account_settings_toggle(callback: CallbackQuery) -> None:
    ...
    if key == "auto_confirm":
        enabled = await db.get_bool_account_setting(callback.from_user.id, "auto_confirm")
        await clean_callback_answer(
            callback,
            await build_auto_confirm_text(callback.from_user.id),
            reply_markup=auto_confirm_menu_keyboard(enabled),
        )
        return
...
@dp.callback_query(F.data == "autoconfirm:toggle")
async def auto_confirm_toggle(callback: CallbackQuery) -> None:
    enabled = await db.toggle_bool_account_setting(callback.from_user.id, "auto_confirm")
    await clean_callback_answer(
        callback,
        await build_auto_confirm_text(callback.from_user.id),
        reply_markup=auto_confirm_menu_keyboard(enabled),
        alert_text=("Автоподтверждение включено" if enabled else "Автоподтверждение выключено"),
    )


@dp.callback_query(F.data == "autoconfirm:select_products")
async def auto_confirm_select_products(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer("Ищу твои товары…")
    try:
        await callback.message.edit_text(
            "🔎 <b>Ищу товары для автоподтверждения</b>\\n\\n"
            "Проверяю твои активные товары Starvell."
        )
    except Exception:
        pass

    try:
        _account, offers = await load_all_seller_products_for_message_settings(
            callback.from_user.id
        )
    except Exception as exc:
        await callback.message.edit_text(
            "❌ <b>Не удалось получить товары профиля</b>\\n\\n"
            f"<code>{escape(str(exc))}</code>",
            reply_markup=auto_confirm_menu_keyboard(
                await db.get_bool_account_setting(callback.from_user.id, "auto_confirm")
            ),
        )
        return
    ...
    if not offers:
        await callback.message.edit_text(
            "❌ В JSON профиля Starvell не найдено доступных товаров.",
            reply_markup=auto_confirm_menu_keyboard(
                await db.get_bool_account_setting(callback.from_user.id, "auto_confirm")
            ),
        )
    ...
    await state.update_data(auto_confirm_product_picker=offers)
    await callback.message.edit_text(
        "📦 <b>Выбери товар для автоподтверждения</b>\\n\\n"
        f"Найдено товаров: <b>{len(offers)}</b>",
        reply_markup=build_message_products_keyboard(
            offers,
            action_prefix="autoconfirmpick",
        ),
    )


@dp.callback_query(F.data.startswith("autoconfirmpick:"))
async def auto_confirm_product_pick(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    command = parts[1]
    data = await state.get_data()
    offers = data.get("auto_confirm_product_picker") or []

    if command == "page" and len(parts) >= 3:
        page = int(parts[2])
    ...
    if command != "select" or len(parts) < 3:
        await callback.answer()
        return
    ...
    selected = next(
        (
            offer
            for offer in offers
            if str(offer.get("publicId") or offer.get("offerPublicId") or "") == offer_id
        ),
        None,
    )
    if not selected:
    ...
    current = csv_set_from_text(
        await db.get_account_setting(callback.from_user.id, "auto_confirm_offer_ids", "")
    )
    if offer_id in current:
        current.remove(offer_id)
        action_text = "Товар убран"
    else:
    ...
    await db.set_account_setting(
        callback.from_user.id,
        "auto_confirm_offer_ids",
        ",".join(sorted(current)),
    )
    await callback.answer(action_text)
    await callback.message.edit_text(
        "📦 <b>Выбор товаров для автоподтверждения</b>\\n\\n"
        f"Выбрано товаров: <b>{len(current)}</b>\\n"
        f"Последний товар: <b>{escape(get_offer_title(selected))}</b>",
        reply_markup=build_message_products_keyboard(
            offers,
            action_prefix="autoconfirmpick",
        ),
...
@dp.callback_query(F.data == "autoconfirm:clear_products")
async def auto_confirm_clear_products(callback: CallbackQuery) -> None:
    await db.set_account_setting(callback.from_user.id, "auto_confirm_offer_ids", "")
    enabled = await db.get_bool_account_setting(callback.from_user.id, "auto_confirm")
    await clean_callback_answer(
        callback,
        await build_auto_confirm_text(callback.from_user.id),
        reply_markup=auto_confirm_menu_keyboard(enabled),
        alert_text="Выбор товаров очищен",
    )
...
async def product_select_menu_removed(callback: CallbackQuery) -> None:
    await callback.answer("Автоответчик удалён из бота.", show_alert=True)
...
async def account_settings_text_view(callback: CallbackQuery) -> None:
    ...
    await clean_callback_answer(
        callback,
        await build_account_setting_text_view(callback.from_user.id, key),
...
@dp.callback_query(F.data == "reminder:find_chats")
async def confirm_reminder_find_chats(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.answer("Ищу чаты без подтверждения…")
    try:
        await callback.message.edit_text(
            "🔎 <b>Ищу чаты без подтверждения</b>\n\n"
            "Получаю список открытых заказов одним запросом Starvell."
        )
    except Exception:
        pass
    ...
    if not account:
        await callback.message.edit_text(
            "❌ Сначала подключи аккаунт Starvell.",
            reply_markup=confirm_reminder_menu_keyboard(),
        )
        return
    ...
    try:
        chats = await asyncio.wait_for(
            find_unconfirmed_chats_for_account(account),
            timeout=70,
        )
    except asyncio.TimeoutError:
        await callback.message.edit_text(
            "❌ <b>Поиск превысил 70 секунд</b>\n\n"
            "Starvell не ответил на list-open-order ни через прокси, ни напрямую.",
            reply_markup=confirm_reminder_menu_keyboard(),
        )
        return
    except Exception as exc:
        await callback.message.edit_text(
            "❌ <b>Не удалось проверить чаты</b>\n\n"
            f"<code>{escape(str(exc))}</code>",
            reply_markup=confirm_reminder_menu_keyboard(),
        )
        return
    ...
    if not chats:
        await callback.message.edit_text(
            "✅ Сейчас не найдено чатов, где заказ оплачен, но получение ещё не подтверждено.",
            reply_markup=confirm_reminder_menu_keyboard(),
        )
        return

    await state.update_data(confirm_reminder_chats=chats)
    await callback.message.edit_text(
        "💬 <b>Чаты без подтверждения</b>\n\n"
        f"Найдено: <b>{len(chats)}</b>\n\n"
        "Выбери чат, для которого нужно настроить напоминание:",
        reply_markup=build_unconfirmed_chats_keyboard(chats),
    )


@dp.callback_query(F.data == "confirm_reminder_back")
async def confirm_reminder_back(callback: CallbackQuery) -> None:
    await clean_callback_answer(
        callback,
        await build_confirm_reminder_text(callback.from_user.id),
        reply_markup=confirm_reminder_menu_keyboard(),
...
@dp.callback_query(F.data.startswith("confirm_reminder_chat:"))
async def confirm_reminder_select_chat(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    chat_id = str(callback.data or "").split(":", 1)[1]
    data = await state.get_data()
    chats = data.get("confirm_reminder_chats") or []
    selected = next(
        (item for item in chats if str(item.get("chatId") or "") == chat_id),
        None,
    )
    if not selected:
    ...
    title = str(selected.get("title") or chat_id)
    await state.update_data(
        selected_confirm_reminder_chat_id=chat_id,
        selected_confirm_reminder_chat_title=title,
    )

    current_text = await db.get_account_setting(
        callback.from_user.id,
        f"confirm_reminder_chat_text:{chat_id}",
        "",
    )
    time_value = await db.get_account_setting(
        callback.from_user.id,
        f"confirm_reminder_chat_time:{chat_id}",
        "13:00",
    )
    period_days = int(
        await db.get_account_setting(
            callback.from_user.id,
            f"confirm_reminder_chat_period_days:{chat_id}",
            "1",
        ) or "1"
    )
    if period_days == 1:
    ...
    await callback.message.edit_text(
        "⏰ <b>Напоминание о подтверждении</b>\n\n"
        f"Чат: <b>{escape(title)}</b>\n"
        f"Время отправки: <b>{escape(str(time_value))}</b>\n"
        f"Дни: <b>{escape(period_text)}</b>\n"
        "Задержка после заказа: <b>только спустя 24 часа</b>\n\n"
        "<b>Текущий текст:</b>\n"
        f"{escape(current_text) if current_text else 'текст не задан'}",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="🕐 Выбрать время", callback_data=f"reminder_chat:time_menu:{chat_id}")],
                [InlineKeyboardButton(text="📅 Выбрать дни", callback_data=f"reminder_chat:period_menu:{chat_id}")],
                [InlineKeyboardButton(text="✏️ Изменить текст", callback_data=f"reminder_chat:edit:{chat_id}")],
                [InlineKeyboardButton(text="🗑 Очистить текст", callback_data=f"reminder_chat:clear:{chat_id}")],
                [InlineKeyboardButton(text="‹ Назад к чатам", callback_data="reminder:find_chats")],
            ]
        ),
    )
    await callback.answer()
...
@dp.callback_query(F.data.startswith("reminder_chat:time_menu:"))
async def confirm_reminder_chat_time_menu(callback: CallbackQuery) -> None:
    ...
    await clean_callback_answer(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="09:00", callback_data=f"reminder_chat:time:{chat_id}:09:00"),
                    InlineKeyboardButton(text="13:00", callback_data=f"reminder_chat:time:{chat_id}:13:00"),
                    InlineKeyboardButton(text="18:00", callback_data=f"reminder_chat:time:{chat_id}:18:00"),
                ],
                [
                    InlineKeyboardButton(text="21:00", callback_data=f"reminder_chat:time:{chat_id}:21:00"),
                    InlineKeyboardButton(text="‹ Назад", callback_data=f"confirm_reminder_chat:{chat_id}"),
                ],
            ]
        ),
    )
...
@dp.callback_query(F.data.startswith("reminder_chat:time:"))
async def confirm_reminder_chat_time_set(callback: CallbackQuery) -> None:
    parts = callback.data.split(":")
    if len(parts) < 4:
        await callback.answer()
        return
    chat_id = parts[2]
    time_value = parts[3] + (":" + parts[4] if len(parts) > 4 else "")
    await db.set_account_setting(
        callback.from_user.id,
        f"confirm_reminder_chat_time:{chat_id}",
        time_value,
...
@dp.callback_query(F.data.startswith("reminder_chat:period_menu:"))
async def confirm_reminder_chat_period_menu(callback: CallbackQuery) -> None:
    chat_id = callback.data.split(":", 2)[2]
    await clean_callback_answer(
        callback,
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="Каждый день", callback_data=f"reminder_chat:period:{chat_id}:1")],
                [InlineKeyboardButton(text="Спустя день", callback_data=f"reminder_chat:period:{chat_id}:2")],
                [InlineKeyboardButton(text="Раз в 3 дня", callback_data=f"reminder_chat:period:{chat_id}:3")],
                [InlineKeyboardButton(text="Раз в неделю", callback_data=f"reminder_chat:period:{chat_id}:7")],
                [InlineKeyboardButton(text="‹ Назад", callback_data=f"confirm_reminder_chat:{chat_id}")],
            ]
        ),
    )
...
@dp.callback_query(F.data.startswith("reminder_chat:period:"))
async def confirm_reminder_chat_period_set(callback: CallbackQuery) -> None:
    parts = callback.data.split(":")
    if len(parts) < 4:
        await callback.answer()
        return
...
@dp.callback_query(F.data.startswith("reminder_chat:edit:"))
async def confirm_reminder_chat_text_edit(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    chat_id = callback.data.split(":", 2)[2]
    await state.update_data(selected_confirm_reminder_chat_id=chat_id)
    await state.set_state(MessageAutomationStates.waiting_for_confirm_reminder_text)
    await callback.message.answer(
        "✍️ Введи текст напоминания для этого чата.\n\n"
        "Можно использовать переменные:\n"
        "<code>{username}</code>, <code>{order_id}</code>, <code>{seller}</code>",
        reply_markup=cancel_keyboard(),
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("reminder_chat:clear:"))
async def confirm_reminder_chat_text_clear(callback: CallbackQuery) -> None:
    chat_id = callback.data.split(":", 2)[2]
    await db.set_account_setting(
        callback.from_user.id,
        f"confirm_reminder_chat_text:{chat_id}",
    ...
    await callback.answer("Текст очищен")
    await callback.message.edit_reply_markup(
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="‹ Назад", callback_data=f"confirm_reminder_chat:{chat_id}")]
...
async def account_settings_text_save(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        "✅ Текст сохранён.\n\n" + await build_account_setting_text_view(message.from_user.id, key),
        reply_markup=account_setting_text_keyboard(key),
    )
...
@dp.message(F.text == "/message_automations_products")
async def message_automations_products(message: Message) -> None:
    rows = [
        [InlineKeyboardButton(text="📦 Автовыдача", callback_data="msg_auto:autodelivery")],
        [InlineKeyboardButton(text="💬 Автоответ", callback_data="msg_auto:autoreply")],
        [InlineKeyboardButton(text="✅ Напоминание о подтверждении", callback_data="msg_auto:confirm")],
    ]
    await message.answer(
        "💬 <b>Автоматические сообщения</b>\n\n"
        "Во всех настройках можно выбрать конкретный товар из твоих товаров Starvell.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
    )


@dp.callback_query(F.data.startswith("msg_auto:"))
async def message_automation_entry(callback: CallbackQuery, state: FSMContext) -> None:
    action = str(callback.data or "").split(":", 1)[1]
    if action == "confirm":
        account = await get_primary_account(callback.from_user.id)
        if not account:
            await callback.answer("Сначала подключи аккаунт Starvell.", show_alert=True)
            return

        await callback.message.edit_text("🔎 Ищу чаты без подтверждения…")
        chats = await find_unconfirmed_chats_for_account(account)
        if not chats:
            await callback.message.edit_text(
                "✅ Сейчас не найдено чатов, где заказ оплачен, но получение ещё не подтверждено."
            )
    ...
    if not offers:
        await callback.message.edit_text("Твои активные товары не найдены ни в категориях, ни в личных данных профиля Starvell.")
        await callback.answer()
        return

    await state.update_data(
        message_automation_action=action,
        message_automation_offers=offers,
    )
    await callback.message.edit_text(
        "📦 <b>Выбери товар</b>\n\n"
        "Товары найдены перебором всех 35 категорий Starvell.",
        reply_markup=build_message_products_keyboard(
            offers,
            action_prefix=f"msg_product:{action}",
        ),
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("msg_product:"))
async def message_automation_product_callback(
    callback: CallbackQuery,
    state: FSMContext,
) -> None:
    parts = str(callback.data or "").split(":")
    ...
    command = parts[2]
    data = await state.get_data()
    offers = data.get("message_automation_offers") or []

    if command == "page" and len(parts) >= 4:
        page = int(parts[3])
        await callback.message.edit_reply_markup(
            reply_markup=build_message_products_keyboard(
                offers,
    ...
    if command == "back":
        await callback.message.edit_text(
            "Настройка отменена. Открой раздел автоматических сообщений заново."
        )
        await callback.answer()
        return

    if command != "select" or len(parts) < 4:
        await callback.answer()
        return
    ...
    selected = next(
        (
            offer
            for offer in offers
            if str(offer.get("publicId") or offer.get("offerPublicId") or "") == public_id
        ),
        None,
    )
    if not selected:
    ...
    await state.update_data(
        selected_message_product_id=public_id,
        selected_message_product_title=get_offer_title(selected),
        selected_message_action=action,
    )

    await callback.message.edit_text(
        "✍️ <b>Введи текст сообщения</b>\n\n"
        f"Товар: <b>{escape(get_offer_title(selected))}</b>\n\n"
        "Отправь текст одним сообщением."
    )
    await state.set_state(MessageAutomationStates.waiting_for_product_message)
    await callback.answer()


@dp.message(MessageAutomationStates.waiting_for_product_message)
async def save_product_message_automation(
    message: Message,
    state: FSMContext,
) -> None:
    ...
    data = await state.get_data()
    action = str(data.get("selected_message_action") or "")
    public_id = str(data.get("selected_message_product_id") or "")
    title = str(data.get("selected_message_product_title") or "Товар")

    key = f"message_automation:{action}:{public_id}"
    await db.set_user_setting(message.from_user.id, key, text)
    await state.clear()

    labels = {
        "autodelivery": "автовыдачи",
        "autoreply": "автоответа",
    }
    await message.answer(
        "✅ Сообщение сохранено.\n\n"
        f"Настройка: <b>{labels.get(action, action)}</b>\n"
        f"Товар: <b>{escape(title)}</b>"
    )
...
async def confirm_reminder_select_chat(
    ...
    await state.update_data(
        selected_confirm_reminder_chat_id=chat_id,
        selected_confirm_reminder_chat_title=selected.get("title") or chat_id,
    )
    await callback.message.edit_text(
        "✍️ <b>Введи текст напоминания</b>\n\n"
        f"Чат: <b>{escape(str(selected.get('title') or chat_id))}</b>\n\n"
        "Бот сохранит этот текст для выбранного чата."
    )
    await state.set_state(MessageAutomationStates.waiting_for_confirm_reminder_text)
    await callback.answer()


@dp.message(MessageAutomationStates.waiting_for_confirm_reminder_text)
async def save_confirm_reminder_text(
    message: Message,
    state: FSMContext,
) -> None:
    text = str(message.text or "").strip()
    ...
    data = await state.get_data()
    chat_id = str(data.get("selected_confirm_reminder_chat_id") or "")
    title = str(data.get("selected_confirm_reminder_chat_title") or chat_id)

    if not chat_id:
        await state.clear()
        await message.answer(
            "❌ Не удалось определить выбранный чат. Открой список чатов и выбери его заново."
        )
        return
    ...
    try:
        ...
        await message.answer(
            "❌ Не удалось сохранить текст напоминания.\n\n"
            f"<code>{escape(str(exc))}</code>"
        )
        return
    ...
    await message.answer(
        "✅ <b>Напоминание сохранено</b>\n\n"
        f"Чат: <b>{escape(title or chat_id)}</b>\n"
        f"Текст: {escape(text)}",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="‹ Вернуться к настройкам чата",
                        callback_data=f"confirm_reminder_chat:{chat_id}",
                    )
                ]
            ]
...
@dp.message(F.text == "/autodelivery_full_scan_debug")
async def autodelivery_full_scan_debug(message: Message) -> None:
    account = await get_primary_account(message.from_user.id)
    if not account:
        await message.answer("Сначала подключи аккаунт Starvell.")
        return

    username = str(account.username or "").strip()
    if not username:
        await message.answer("У подключённого аккаунта не найден username.")
        return
    ...
    try:
        ...
        await message.answer(
            "❌ Ошибка полного поиска товаров для автовыдачи:\n"
            f"<code>{escape(str(exc))}</code>"
        )
        return

    lines = [
        "🧪 <b>Полный поиск товаров для автовыдачи</b>",
        "",
        f"Версия: <code>{AUTO_DELIVERY_FULL_SCAN_VERSION}</code>",
        f"Проверено категорий: <b>{len(AUTO_DELIVERY_CATEGORY_PAGES)}</b>",
        f"Найдено твоих товаров: <b>{len(offers)}</b>",
        "",
    ]
    for offer in offers[:50]:
        title = (
            ((offer.get("descriptions") or {}).get("rus") or {}).get("briefDescription")
            or offer.get("_categoryTitle")
            or "Товар"
        )
        public_id = offer.get("publicId") or offer.get("offerPublicId") or "—"
        lines.append(f"• {escape(str(title)[:80])} — <code>{escape(str(public_id))}</code>")
...
@dp.message(F.text == "/autoraise_all_categories_debug")
async def autoraise_all_categories_debug(message: Message) -> None:
    lines = [
        "🧪 <b>Каталог автоподнятия</b>",
        "",
        f"Версия: <code>{AUTO_RAISE_ALL_CATEGORIES_VERSION}</code>",
        f"Игр: <b>{len(AUTO_RAISE_CATEGORY_CATALOG)}</b>",
        f"Категорий: <b>{sum(len(v) for v in AUTO_RAISE_CATEGORY_CATALOG.values())}</b>",
        "",
    ]
    for game_id, category_ids in sorted(AUTO_RAISE_CATEGORY_CATALOG.items()):
        for category_id in category_ids:
            title = AUTO_RAISE_CATEGORY_NAMES.get((game_id, category_id), "Без названия")
            lines.append(f"• {escape(title)} — <code>{game_id}/{category_id}</code>")
    await message.answer("\n".join(lines))
