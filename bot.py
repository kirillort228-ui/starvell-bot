...
async def load_all_seller_products_for_message_settings(
    ...
    account = await db.get_primary_account(telegram_user_id)
    if not account:
    ...
    client = build_starvell_client(account)
...
async def confirm_reminder_choose_product(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not offers:
        await clean_callback_answer(
            callback,
            "❌ Твои активные товары не найдены.",
            reply_markup=confirm_reminder_menu_keyboard(),
        )
        return
...
async def message_automation_entry(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if action == "confirm":
        account = await db.get_primary_account(callback.from_user.id)
        if not account:
    ...
    if not offers:
        await callback.message.edit_text("Твои активные товары не найдены.")
        await callback.answer()
        return
...
async def autodelivery_full_scan_debug(message: Message) -> None:
    account = await db.get_primary_account(message.from_user.id)
    if not account:
