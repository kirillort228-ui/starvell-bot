...
class Database:
        ...
        async with aiosqlite.connect(self.path) as db:
            ...
            for column_sql in (
                ...
                try:
                    ...
                    pass
            try:
                await db.execute("ALTER TABLE topup_requests ADD COLUMN crypto_invoice_id TEXT")
            except Exception:
                pass
            try:
                await db.execute("ALTER TABLE topup_requests ADD COLUMN crypto_pay_url TEXT")
            except Exception:
                pass
            await db.commit()
    ...
    async def touch_user(self, user_id: int, username: str | None = None, first_name: str | None = None) -> None:
        await self.ensure_user(user_id)
        now = datetime.now(timezone.utc).isoformat()
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                "UPDATE users SET telegram_username=?, first_name=?, last_seen_at=? WHERE user_id=?",
                (username, first_name, now, int(user_id)),
            )
            await db.commit()

    async def get_user_public_info(self, user_id: int) -> dict[str, Any]:
        await self.ensure_user(user_id)
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute(
                "SELECT user_id, telegram_username, first_name, subscription_until, bot_balance_kopecks, created_at, last_seen_at FROM users WHERE user_id=?",
                (int(user_id),),
            )
            row = await cur.fetchone()
            return dict(row) if row else {}

    async def can_create_topup_request(self, user_id: int, max_pending: int = 3, cooldown_seconds: int = 60) -> tuple[bool, str]:
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute("SELECT COUNT(*) AS c FROM topup_requests WHERE user_id=? AND status='pending'", (int(user_id),))
            row = await cur.fetchone()
            if row and int(row['c'] or 0) >= max_pending:
                return False, f"У тебя уже есть {max_pending} неоплаченных счёта."
            cur = await db.execute("SELECT created_at FROM topup_requests WHERE user_id=? ORDER BY id DESC LIMIT 1", (int(user_id),))
            row = await cur.fetchone()
            if row and row['created_at']:
                try:
                    created = datetime.fromisoformat(str(row['created_at']))
                    left = cooldown_seconds - (datetime.now(timezone.utc) - created.astimezone(timezone.utc)).total_seconds()
                    if left > 0:
                        return False, f"Подожди ещё {max(1, int(left))} сек."
                except Exception:
                    pass
        return True, ""
    ...
    async def get_admin_dashboard_stats(self) -> dict[str, Any]:
        ...
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            async def scalar(q, params=()):
                cur = await db.execute(q, params); row = await cur.fetchone(); return int(row[0] or 0) if row else 0
            total_users = await scalar("SELECT COUNT(*) FROM users")
            active_subscriptions = await scalar("SELECT COUNT(*) FROM users WHERE subscription_until IS NOT NULL AND subscription_until > ?", (now,))
            connected_accounts = await scalar("SELECT COUNT(*) FROM accounts")
            notifications_enabled = await scalar("SELECT COUNT(*) FROM accounts WHERE notifications_enabled=1")
            total_invoices = await scalar("SELECT COUNT(*) FROM topup_requests")
            paid_invoices = await scalar("SELECT COUNT(*) FROM topup_requests WHERE status='approved'")
            pending_invoices = await scalar("SELECT COUNT(*) FROM topup_requests WHERE status='pending'")
            rejected_invoices = await scalar("SELECT COUNT(*) FROM topup_requests WHERE status='rejected'")
            paid_amount_kopecks = await scalar("SELECT COALESCE(SUM(amount_kopecks),0) FROM topup_requests WHERE status='approved'")
            cur = await db.execute("SELECT user_id, telegram_username, first_name FROM users ORDER BY COALESCE(last_seen_at, created_at) DESC LIMIT 10")
            recent_users = [dict(r) for r in await cur.fetchall()]
            return locals() | {'recent_users': recent_users}
    ...
    async def set_topup_crypto_invoice(self, request_id: int, invoice_id: str, pay_url: str) -> None:
        now = datetime.now(timezone.utc).isoformat()
        async with aiosqlite.connect(self.path) as db:
            await db.execute(
                """
                UPDATE topup_requests
                SET crypto_invoice_id = ?, crypto_pay_url = ?, updated_at = ?
                WHERE id = ?
                """,
                (str(invoice_id), str(pay_url), now, int(request_id)),
            )
            await db.commit()

    async def get_topup_by_crypto_invoice(self, invoice_id: str) -> TopUpRequest | None:
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                "SELECT * FROM topup_requests WHERE crypto_invoice_id = ?",
                (str(invoice_id),),
            )
            row = await cursor.fetchone()
            if not row:
    ...
    async def list_pending_crypto_topups(self) -> list[dict[str, Any]]:
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                """
                SELECT id, user_id, amount_kopecks, status, crypto_invoice_id, crypto_pay_url
                FROM topup_requests
