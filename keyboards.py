...
def auto_raise_settings_keyboard(enabled: bool = False) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="📋 Выбрать товары", callback_data="autoraise:scan_products")],
            [InlineKeyboardButton(text="🧹 Очистить выбор товаров", callback_data="autoraise:clear_products")],
            [InlineKeyboardButton(text="⏱ Интервал", callback_data="autoraise:interval_menu")],
            [InlineKeyboardButton(text="🚀 Поднять выбранные лоты сейчас", callback_data="autoraise:run_now")],
            [InlineKeyboardButton(text="‹ Назад к автонастройкам", callback_data="accset:menu")],
        ]
    )
...
def auto_delivery_products_keyboard(items: list[dict]) -> InlineKeyboardMarkup:
    rows = []
    for idx, item in enumerate(items[:12]):
        title = str(
            item.get("title")
    ...
    rows.append([InlineKeyboardButton(text="‹ Назад", callback_data="autodelivery:menu")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
