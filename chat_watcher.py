...
def chat_event_setting_key(last_message: dict) -> str | None:
    notification_type = get_notification_type(last_message).upper()
    content = str(last_message.get("content") or "").lower()

    if notification_type == "ORDER_SELLER_COMPLETED":
        return "after_seller_confirm"

    if notification_type in {
        "ORDER_COMPLETED",
        "ORDER_BUYER_COMPLETED",
        "ORDER_CLIENT_COMPLETED",
        "ORDER_CONFIRMED",
    }:
    ...
    if any(word in content for word in ("возврат", "спор", "проблем", "отмен")):
        return "problem_text"
...
async def maybe_send_event_auto_message(client: StarvellClient, db: Database, account: StarvellAccount, chat: dict, last_message: dict, *, my_user_id: int | None, my_username: str) -> None:
    ...
    if not setting_key:
        logger.warning(
            "AUTO_MESSAGE_SKIP account_id=%s chat_id=%s reason=event_not_mapped notification_type=%s",
            account.id,
            chat.get("id"),
    ...
    if await db.get_account_setting(account.user_id, dedupe_key):
        logger.warning(
            "AUTO_MESSAGE_SKIP account_id=%s chat_id=%s reason=deduplicated key=%s",
            account.id,
            chat_id,
            setting_key,
        )
    ...
    if not template or not template.strip():
        logger.warning(
            "AUTO_MESSAGE_SKIP account_id=%s chat_id=%s reason=text_not_configured key=%s",
            account.id,
            chat_id,
            setting_key,
        )
    ...
    result = await client.send_chat_message(chat_id, rendered)
    await db.set_account_setting(account.user_id, dedupe_key, "1")
    logger.warning(
        "AUTO_MESSAGE_SUCCESS account_id=%s chat_id=%s key=%s notification_type=%s result=%s",
        account.id,
        chat_id,
        setting_key,
        get_notification_type(last_message),
