...
class StarvellClient:
    ...
    async def get_trade_page(self, game_slug: str, category_slug: str | None = None) -> dict:
        ...
        url = f"{self.base_url}/_next/data/{build_id}/trade.json"
        return await self._json_request(url, params=params)


    async def get_trade_page_robust(self, game_slug: str, category_slug: str | None = None) -> dict:
        """
        Tries several Starvell representations of a trade page:
        1. Next.js JSON route;
        2. public HTML category page and its embedded __NEXT_DATA__;
        3. alternate /trade path.
        """
        errors = []
        ...
        raise StarvellApiError(
            "Не удалось открыть страницу товаров Starvell: " + "; ".join(errors[-3:])
        )
    ...
    async def collect_bump_groups_from_open_trade(self, my_user_id: int | None = None) -> dict[int, set[int]]:
        ...
        while pages_to_scan and pages_checked < 80:
            ...
            try:
                data = await self.get_trade_page(game_slug, category_slug)
            except Exception:
                continue
    ...
    async def collect_active_seller_offers(self, username: str | None = None) -> list[dict]:
        """
        Finds active offers that belong to the connected seller account.

        Sources:
        1. authenticated account/offers endpoints;
        2. public profile JSON and profile HTML;
        3. authenticated trade.json pages for every category of discovered games.

        Only active, visible and approved offers owned by the current seller are returned.
        """
        seller_username = str(username or "").strip()
        seller_username_l = seller_username.lower()
        seller_user_id: int | None = None
        seller_identity_debug = []
        datasets: list[dict] = []

        # Resolve current seller ID from authenticated pages.
        for getter in (
            self.get_chats,
        ...
        # Account offers endpoint variants.
        try:
            datasets.append(await self.get_account_offers())
        except Exception:
            pass

        # Public profile HTML may contain offers even when profile JSON does not.
        if seller_username:
            try:
        ...
        def offer_owner_matches(obj: dict) -> bool:
            ...
            # Authenticated account/offers/profile payloads may omit the owner on each offer.
            return owner_id is None and not owner_username

        def extract_offers(data: dict) -> list[dict]:
            result: list[dict] = []
            for obj in _iter_dicts(data):
                if not isinstance(obj, dict):
                    continue
                ...
                if not public_id or not game_id or not category_id:
                    continue
                if not offer_owner_matches(obj):
                    continue
                if obj.get("isActive") is False or obj.get("isHidden") is True:
                    continue
                ...
                if moderation and moderation not in {"APPROVED", "ACTIVE", "PUBLISHED"}:
                    continue
        ...
        found: dict[str, dict] = {}
        discovered_games: dict[int, str] = {}

        for data in datasets:
            for item in extract_offers(data):
            ...
            try:
                ...
                if game_id and game_slug:
                    discovered_games[game_id] = game_slug
            except Exception:
                pass

        # Steam is known from the connected account's trade.json and is cheap to scan.
        discovered_games.setdefault(16, "steam")

        # Scan category pages concurrently. This is much faster than the old 80-page fallback.
        for _, game_slug in list(discovered_games.items())[:8]:
            try:
                base_data = await self.get_trade_page_robust(game_slug)
            except Exception:
                continue

            for item in extract_offers(base_data):
                found[item["offerPublicId"]] = item
            ...
            if game_slug == "steam":
                for known_slug in (
                    ...
                    if known_slug not in category_slugs:
                        category_slugs.append(known_slug)
            ...
            category_pages = await asyncio.gather(
                *(fetch_category(slug) for slug in category_slugs[:30]),
