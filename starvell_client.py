...
FALLBACK_NEXT_BUILD_ID = "2WjfKEE-M4CVxU_chK3Jt"
...
class StarvellClient:
    ...
    async def get_build_id(
        ...
        *,
        ...
        """Получает актуальный Next.js buildId без вызова несуществующего _request."""
        if self._build_id and not force_refresh:
            return self._build_id

        candidate_urls = []
        if profile_username:
            candidate_urls.append(f"{self.base_url}/profile/{profile_username}")
        candidate_urls.append(f"{self.base_url}/chat")
        candidate_urls.append(f"{self.base_url}/")
        ...
        self._build_id = FALLBACK_NEXT_BUILD_ID
        return self._build_id

    async def get_chat_page(self, interlocutor_id: int) -> dict:
        """Получает данные чата и активные заказы продавца."""
        response = await self.client.post(
            f"{self.base_url}/api/bff/chat-page",
            json={"interlocutorId": int(interlocutor_id)},
            headers={
        ...
        if response.status_code >= 400:
            raise StarvellApiError(
                f"chat-page вернул HTTP {response.status_code}: {response.text[:400]}"
            )
        try:
            data = response.json()
        except Exception as exc:
            raise StarvellApiError(
                f"chat-page вернул не JSON: {response.text[:300]}"
            ) from exc
        return data if isinstance(data, dict) else {}

    async def mark_seller_completed(self, order_id: str) -> dict:
        """Подтверждает выполнение заказа продавцом."""
        order_id = str(order_id or "").strip()
        if not order_id:
            raise StarvellApiError("order_id не указан.")

        response = await self.client.post(
            f"{self.base_url}/api/orders/{order_id}/mark-seller-completed",
            json={"id": order_id},
            headers={
                "Accept": "application/json, text/plain, */*",
                "Content-Type": "application/json",
        ...
        if response.status_code >= 400:
            raise StarvellApiError(
                f"mark-seller-completed вернул HTTP {response.status_code}: "
                f"{response.text[:400]}"
            )
        ...
        try:
            ...
            return {"ok": True, "status": response.status_code, "text": response.text[:400]}
        return data if isinstance(data, dict) else {"ok": True, "data": data}


    async def get_open_order_chats(
        self,
        *,
        offset: int = 0,
        limit: int = 50,
        ...
        """Получает открытые заказы с теми же заголовками, что использует сайт."""
        url = f"{self.base_url}/api/chats/list-open-order"
        payload = {
            "offset": int(offset),
            "limit": int(limit),
            "role": str(role),
        ...
        try:
            ...
            raise StarvellApiError(f"Прокси не отвечает: {exc}") from exc
        except httpx.TimeoutException as exc:
            raise StarvellApiError(
                "Starvell не ответил на list-open-order за 30 секунд."
            ) from exc
        except httpx.RequestError as exc:
            raise StarvellApiError(
                f"Ошибка запроса list-open-order: {type(exc).__name__}: {exc}"
            ) from exc

        if response.status_code >= 400:
            raise StarvellApiError(
                f"list-open-order вернул HTTP {response.status_code}: "
                f"{response.text[:500]}"
            )
        ...
        try:
            ...
            raise StarvellApiError(
                "list-open-order вернул не JSON: "
                f"{response.text[:300]}"
            ) from exc
        ...
        if isinstance(data, dict):
            for key in ("items", "chats", "data", "result"):
                value = data.get(key)
                if isinstance(value, list):
                    return [item for item in value if isinstance(item, dict)]

        raise StarvellApiError(
            f"Неожиданный ответ list-open-order: {str(data)[:500]}"
        )


    async def get_chats(self) -> dict:
        """Получает актуальный chat.json и обновляет buildId при 404/HTML."""
        last_error: Exception | None = None

        for attempt in range(2):
            build_id = await self.get_build_id(force_refresh=(attempt > 0))
            ...
            try:
                ...
                raise StarvellApiError("Ошибка прокси при загрузке chat.json.") from exc
            except httpx.TimeoutException as exc:
                raise StarvellApiError("Starvell долго не отвечает при загрузке chat.json.") from exc
            except httpx.RequestError as exc:
                raise StarvellApiError(f"Ошибка запроса chat.json: {exc}") from exc
            ...
            if response.status_code == 200 and "json" in content_type:
                try:
                    ...
                    if isinstance(data, dict):
                        return data
                    last_error = StarvellApiError("chat.json вернул неожиданный формат.")
            else:
                last_error = StarvellApiError(
                    f"chat.json вернул HTTP {response.status_code}, "
        ...
        raise StarvellApiError(
            "Не удалось получить chat.json после обновления buildId. "
    ...
    async def collect_profile_page_offers(
        self,
        username: str,
        *,
        ...
        """
        Получает товары напрямую из:
        pageProps.bff.userProfileOffers[].offers

        Это основной и самый надёжный источник товаров профиля Starvell.
        """
        data = await self.get_profile(username)
        page_props = data.get("pageProps") if isinstance(data, dict) else {}
        bff = page_props.get("bff") if isinstance(page_props, dict) else {}
        categories = bff.get("userProfileOffers") if isinstance(bff, dict) else None
        ...
        for category in categories:
            ...
            category_name = str(category.get("name") or "Категория")
            ...
            game_slug = str(game.get("slug") or "")
            ...
            for offer in offers:
                ...
                descriptions = offer.get("descriptions")
                descriptions = descriptions if isinstance(descriptions, dict) else {}
                rus = descriptions.get("rus")
                rus = rus if isinstance(rus, dict) else {}
                ...
                normalized.update(
                    {
                        ...
                        "_categoryTitle": f"{game_name} — {category_name}".strip(" —"),
                        "title": str(title)[:160],
                        "isActive": True,
                    }
                )
    ...
    async def collect_seller_offers_from_category_pages(
        ...
        *,
        ...
        # Получение профиля не должно останавливать весь поиск товаров.
        # Starvell иногда долго отвечает именно на страницу профиля.
        profile = {}
        try:
        ...
        try:
            ...
            build_id = FALLBACK_NEXT_BUILD_ID

        found: dict[str, dict] = {}

        # Сначала пытаемся получить товары из личных источников аккаунта.
        try:
    ...
    async def get_profile(self, username: str) -> dict:
        """
        Загружает JSON профиля. Если buildId устарел и Starvell вернул HTML/404,
        обновляет buildId со страницы профиля и повторяет запрос.
        """
        username = str(username or "").strip()
        if not username:
            raise StarvellApiError("Username профиля не указан.")
        ...
        for attempt in range(2):
            ...
            url = f"{self.base_url}/_next/data/{build_id}/profile/{username.lower()}.json"
            ...
            try:
                response = await self.client.get(
                    ...
                    headers={
                        "Accept": "application/json, text/plain, */*",
                        "X-Nextjs-Data": "1",
                        "Referer": f"{self.base_url}/profile/{username.lower()}",
                    },
                    follow_redirects=True,
                    timeout=httpx.Timeout(20.0, connect=10.0),
                )
            except httpx.ProxyError as exc:
                raise StarvellApiError("Ошибка прокси при загрузке профиля.") from exc
            except httpx.TimeoutException as exc:
                raise StarvellApiError("Starvell долго не отвечает при загрузке профиля.") from exc
            except httpx.RequestError as exc:
                raise StarvellApiError(f"Ошибка запроса профиля: {exc}") from exc
            ...
            last_error = StarvellApiError(
                f"Профиль вернул HTTP {response.status_code}, "
                f"content-type={content_type or 'не указан'}: {body_preview}"
            )

            # 404 HTML почти всегда означает устаревший Next.js buildId.
            self._build_id = None

        raise StarvellApiError(
            "Не удалось получить JSON профиля после обновления buildId. "
            f"Последняя ошибка: {last_error}"
        )
