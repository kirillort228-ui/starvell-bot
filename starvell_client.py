...
class StarvellClient:
    ...
    async def collect_bump_groups_from_open_trade(self, my_user_id: int | None = None) -> dict[int, set[int]]:
        """
        Fallback scanner for public trade pages. It first opens Steam trade page,
        reads categories from pageProps.game.categories, then opens those category pages
        and keeps only offers belonging to my_user_id when userId is present.
        """
        groups: dict[int, set[int]] = {}
        pages_to_scan: list[tuple[str, str | None]] = [("steam", None)]

        try:
            steam_data = await self.get_trade_page("steam")
            for game_id, category_ids in extract_bump_groups_from_data(steam_data, my_user_id=my_user_id).items():
                groups.setdefault(game_id, set()).update(category_ids)

            page_props = steam_data.get("pageProps", {}) if isinstance(steam_data, dict) else {}
            game = page_props.get("game", {}) if isinstance(page_props.get("game"), dict) else {}
            for category in game.get("categories", []) or []:
                slug = category.get("slug")
                if slug:
                    pages_to_scan.append(("steam", str(slug)))
        except Exception:
            pass

        for game_slug, category_slug in pages_to_scan[:20]:
            try:
                data = await self.get_trade_page(game_slug, category_slug)
                for game_id, category_ids in extract_bump_groups_from_data(data, my_user_id=my_user_id).items():
                    groups.setdefault(game_id, set()).update(category_ids)
            except Exception:
                continue
