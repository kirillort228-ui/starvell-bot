...
def _normalize_profile_user_dict(user: dict, requested_username: str | None = None) -> dict:
    ...
    reviews = _first_present(merged, ("reviewsCount", "reviewCount", "reviews", "sellerReviewsCount", "feedbacksCount"))
    ...
    async def collect_marketplace_top_sellers(self, *, max_pages: int = 18, limit: int = 1000) -> list[dict]:
        ...
        for seller in candidate_items:
            ...
            if not username:
                continue
            try:
                profile_user = await self.get_profile_user(username)
                ...
                pass
            # Use marketplace data only as fallback.
            enriched[username.lower()] = seller
