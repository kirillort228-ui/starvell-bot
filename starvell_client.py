...
class StarvellClient:
    ...
    async def get_open_order_chats(
        ...
        *,
        ...
        """Быстро получает чаты с открытыми заказами одним POST-запросом."""
        try:
            response = await self.client.post(
                f"{self.base_url}/api/chats/list-open-order",
                json={
                    "offset": int(offset),
                    "limit": int(limit),
                    "role": str(role),
            ...
            raise StarvellApiError("Запрос списка открытых заказов превысил 10 секунд.") from exc
        except httpx.RequestError as exc:
            raise StarvellApiError(f"Ошибка запроса списка открытых заказов: {exc}") from exc

        if response.status_code >= 400:
            raise StarvellApiError(
                f"Starvell вернул {response.status_code}: {response.text[:300]}"
            )
        ...
        try:
            ...
            raise StarvellApiError(
                f"Starvell вернул не JSON: {response.text[:200]}"
            ) from exc
