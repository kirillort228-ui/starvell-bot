...
class StarvellClient:
    ...
    async def get_top_sellers(self) -> dict:
        candidate_paths = [
            "top", "top-sellers", "top/sellers", "sellers", "sellers/top",
            "users/top", "rating", "leaderboard", "top1000", "top-1000",
        ]

        for path in candidate_paths:
            try:
                data = await self.get_next_data_path(path)
                page_props = data.get("pageProps", {}) if isinstance(data, dict) else {}
                if any(key in page_props for key in ("sellers", "users", "topSellers", "items", "leaders", "profiles")):
                    return data
            except Exception:
                continue

        raise StarvellApiError(
            "Не удалось найти endpoint топа продавцов. "
            "Открой на сайте Starvell страницу топа продавцов, скопируй URL запроса из Network и пришли его."
        )
